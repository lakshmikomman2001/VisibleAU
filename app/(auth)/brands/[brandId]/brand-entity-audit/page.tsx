...
export default async function BrandEntityAuditPage({ params }: { params: Promise<{ brandId: string }> }) {
  ...
  const [techAudit] = await db
    .select({ findings: technicalAudits.findings, scoreBrandEntity: technicalAudits.scoreBrandEntity, crawledAt: technicalAudits.crawledAt })
    ...
    return (
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "40px 32px" }}>
        ...
        <div style={{ padding: 48, textAlign: "center", borderRadius: 8, background: "var(--bg-elevated)", border: "1px solid var(--border-default)" }}>
          <p style={{ fontSize: 14, color: "var(--text-tertiary)" }}>Run a technical audit first.</p>
        </div>
      </div>
    );
  }

  const findings = (techAudit.findings as Record<string, unknown>)?.brandEntity as BrandEntityFindings | undefined;
  ...
  const signals = [
    {
      ...
      detail: findings?.abnNumber ? `ABN: ${findings.abnNumber}` : "No ABN verified",
      pts: 3,
    },
    {
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 600, color: "var(--text-primary)", margin: "0 0 4px" }}>Brand &amp; Entity Audit</h1>
          <p style={{ fontSize: 14, color: "var(--text-secondary)", margin: 0 }}>
            AU-localised brand presence signals &middot; Score: {score}/10
          </p>
        </div>
        <div style={{ fontSize: 36, fontWeight: 600, fontFamily: "var(--font-mono)", color: "var(--text-primary)" }}>{score}/10</div>
      </div>

      {/* Signal Cards */}
      <div style={{ borderRadius: 8, background: "var(--bg-elevated)", border: "1px solid var(--border-default)", overflow: "hidden", marginBottom: 24 }}>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", margin: 0 }}>Entity Signals</h3>
        </div>
        {signals.map((sig) => (
          <div key={sig.label} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
            <span style={{ fontSize: 16 }}>{sig.present ? "✓" : "✗"}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: "var(--text-primary)" }}>{sig.label}</div>
              <div style={{ fontSize: 11, color: "var(--text-tertiary)" }}>{sig.detail}</div>
            </div>
            <span style={{ fontSize: 13, fontFamily: "var(--font-mono)", color: sig.present ? "var(--success)" : "var(--text-tertiary)" }}>
      ...
      {(findings?.directoryPresence?.length ?? 0) > 0 && (
        <div style={{ borderRadius: 8, background: "var(--bg-elevated)", border: "1px solid var(--border-default)", overflow: "hidden" }}>
          <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", margin: 0 }}>AU Directory Presence</h3>
          </div>
          {findings!.directoryPresence.map((dir) => (
            <div key={dir.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
              <span style={{ fontSize: 14 }}>{dir.present ? "✓" : "✗"}</span>
              <span style={{ flex: 1, fontSize: 13, color: "var(--text-primary)" }}>{dir.name}</span>
              <span style={{ fontSize: 11, color: dir.present ? "var(--success)" : "var(--text-tertiary)" }}>
