...
export async function GET(
  ...
  return withRlsContext(currentUser.organizationId, async (tx) => {
    ...
    const entityHomePage = audits.find((a) => a.isEntityHomeCandidate === true) ?? null;

    return NextResponse.json({
      entityHome: entityHomePage,
      entityHomeStatus: entityHomePage
        ? {
            orgSchemaPresent: entityHomePage.entityHomeHasOrgSchema ?? false,
            idFieldPresent: entityHomePage.entityHomeHasIdField ?? false,
            sameAsCount: entityHomePage.entityHomeSameAsCount ?? 0,
            pageUrl: entityHomePage.entityHomePageUrl,
          }
        : null,
      pages: audits,
    });
