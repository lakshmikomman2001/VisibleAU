...
export function EntityHomeCard({ entityHomeStatus }: EntityHomeCardProps) {
    ...
    return (
      ...
      >
        ...
        <p className="mt-2 text-sm" style={{ color: "var(--destructive)" }}>
          We haven&apos;t identified your Entity Home yet &mdash; run an audit to detect the page
          anchoring your brand identity for AI engines.
        </p>
      </div>
    );
  }
  ...
  const isComplete =
    entityHomeStatus.orgSchemaPresent &&
    entityHomeStatus.idFieldPresent &&
    entityHomeStatus.sameAsCount >= 3;

  const gaps: string[] = [];
  if (!entityHomeStatus.orgSchemaPresent)
    gaps.push("Missing Organisation JSON-LD on Entity Home page.");
  if (!entityHomeStatus.idFieldPresent) gaps.push("@id field not pointing to canonical domain.");
  if (entityHomeStatus.sameAsCount < 3)
    gaps.push(`Only ${entityHomeStatus.sameAsCount} sameAs declarations (target: ≥3).`);
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm" style={{ color: "var(--muted)" }}>
        Page found: {entityHomeStatus.pageUrl ?? "unknown"} &mdash;{" "}
        {isComplete ? "Entity Home fully configured." : "Entity Home needs attention."}
      </p>
      ...
      <div className="mt-3 flex flex-wrap gap-4 text-sm">
        <span style={{ color: "var(--foreground)" }}>
          ...
          <strong
            style={{
              color: entityHomeStatus.idFieldPresent ? "var(--success)" : "var(--destructive)",
            }}
          >
        ...
        <span style={{ color: "var(--foreground)" }}>
          ...
          <strong
            style={{
              color: entityHomeStatus.sameAsCount >= 3 ? "var(--success)" : "var(--warning)",
            }}
          >
        ...
        <span style={{ color: "var(--foreground)" }}>
          ...
          <strong
            style={{
              color: entityHomeStatus.orgSchemaPresent ? "var(--success)" : "var(--destructive)",
            }}
          >
