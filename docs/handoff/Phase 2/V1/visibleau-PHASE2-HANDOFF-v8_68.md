...
1. **S8-01 [LLD]** — the `hallucination/acknowledged` fanout PRODUCER emit added at SOURCE (the
...
1. **S8b-01 [LLD]** — `assertBrandAccess(user, brandId)` FORMALISED as the canonical brand-isolation
...
1. **S8b-02 [LLD]** — RBAC matrix role-ceiling rows: change-role + remove-member (owner/admin),
   assign/revoke OWNER role (owner only); admin can’t touch an owner (403).
1. **S8b-03 [LLD]** — audit_trail action enum gains `member_role_changed` + `member_removed`
   (resource_type `org_member`).
1. **S9-02 [PROTOTYPE → FIX 16]** — HealthCheck reconciled to the LLD’s four CROSS-LAYER dimensions
...
Verify v8.68: `grep -m1 '# Version:' visibleau-7layer-lld.md` → 8.68; `grep -c 'FIX 16 (v8.68)' visibleau-prototype-phase2.jsx` → 1; `grep -c 'ATTRIBUTION CORRECTION' visibleau-7layer-lld.md` → 3.
...
1. Phase2Sidebar hubs are intentionally UNGATED; the real entitlement gate is
   BrandIntelTabs. Do not add tierGate to the sidebar.
1. Sidebar ids brand-list / action-center / vertical-packs / billing fall through to
   ‘dashboard’ — PHASE 1 screens (visibleau-prototype.jsx owns them). No duplicate stubs.
1. run-journey.ts (Inngest) vs journey-runner.ts (lib) — correct function/lib split.
1. conversation_journeys mock status ‘ready’/‘not_run’ — harmless dead fixture field.
1. task.source ‘frequency’/‘context’ = valid Phase 1 score DIMENSIONS.
1. White text on accent/gradient FILLS — correct in BOTH themes.
1. text-disabled fails AA — inherited Phase 1 disabled text; WCAG-exempt.
1. contentEditable inline outline:none does NOT suppress the focus ring (box-shadow).
1. LLD code-fence parity is ODD — pre-existing, harmless; viewer limitation on 600KB+.
1. Phase 1 prototype −1 brace — a brace inside a JSX string; Babel-validated.
1. Historical changelog lines are immutable; the live spec is what gets corrected.
1. RETIRED in v8.65 — the former item declaring ${color}30 / ${step.color}30 /
...
1. Fix-note/changelog DESCRIPTIONS quote past corruption literals (FIX-9 backdrop name,
...
1. MetricRow + EmptyState are defined-but-uninstantiated shared-library FORWARD SPEC
   (implementation note 6; mock data is non-empty everywhere). Not dead code.
1. IntelCard `loading` never passed true = the specified loading state; LayerBadge
   size=‘md’ and Phase2TopBar `actions` are optional variants by design.
1. ARIA raw grep counts exceed live counts because the FIX 13 note mentions the
...
1. NEW: AR-02/AR-03/AR-04 fix IDs appear ONLY in the LLD changelog, not as verbatim
...
1. ASSESS THE AUTHORITATIVE DOCUMENT before every fix:
...
1. VERIFY-BEFORE-CLAIM: grep canonical sources; never assert from memory.
1. VERIFY-AFTER-EDIT: re-balance braces/parens (JSX AND the CSS template block), re-check
...
1. VERSION DISCIPLINE: every pass bumps the LLD version + adds a changelog entry naming the
   angle, each fix ID, the assessment, and the CLEAN list. Prototype edits add a FIX N note.
1. Changelog entries are immutable history — never rewrite old entries.
1. Deliver outputs as files + zip; remind Sri that container outputs do NOT persist across chats.
...
1. Phase 2 sprint-prompt design — the LLD is hardened (46 passes, including one
...
1. Responsive breakpoints for the Phase 2 prototype — the one remaining substantive design
   gap (fixed grids, no sm:/md: variants); treat as a dedicated task, not an audit pass.
1. Optional: refresh CLAUDE.md/Foundations to retire the Clerk text (close drift C-04).
1. Audit fatigue: 46 passes in, including a fresh-eyes cross-review that found one real
