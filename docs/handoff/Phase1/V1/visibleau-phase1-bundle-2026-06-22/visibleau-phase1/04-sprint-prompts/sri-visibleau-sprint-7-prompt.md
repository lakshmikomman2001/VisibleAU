...
inngest/functions/
├── technical-audit-run.ts                    # NEW long-running orchestrator (~3-5 min/brand). Triggered by SAME `audit.run` + `{ auditId }` Inngest event as Sprint 3's run-audit.ts (corrected from audit/start — see D-05 v8.69). Runs in parallel; persists to `technical_audits` table; emits `technical-audit.complete` event the audit-completion email handler listens for.
...
export const technicalAudits = pgTable('technical_audits', {
  ...
  // Links to the parallel multidim audit fired by the same `audit.run` event (corrected from audit/start — see D-05 v8.69) (v2.2 third-pass-fix B2).
...
5. AUDIT ORCHESTRATION
   ...
   - **EE1 fix — `technical-audit-run.ts` Inngest function body never specified (same gap as Sprint 6 DD2):**
     ...
     export const technicalAuditRun = inngest.createFunction(
       ...
       { event: 'audit.run' },  // corrected from audit/start — see D-05 v8.69
       async ({ event, step }) => {
     ...
     export const { GET, POST, PUT } = serve({
       ...
       functions: [
         ...
         technicalAuditRun,         // Sprint 7 — listens on audit.run (corrected from audit/start — see D-05 v8.69) (fires in parallel with runAudit)
     ...
     ```
     Both `runAudit` and `technicalAuditRun` listen to `{ event: 'audit.run' }` (corrected from audit/start — see D-05 v8.69). Inngest correctly routes one event to all matching functions — both fire in parallel.

     **EB3 fix — `audit.run` event payload never specified (corrected from audit/start — see D-05 v8.69). `technical-audit-run.ts` needs the brand domain to crawl:**

     The `audit.run` event payload (fired by Sprint 4's "Run Audit" button handler):
     ```typescript
...
- v2.1 (13 May 2026): **Second-pass-fix audit N3.** Added explicit audit-flow design decision to header: technical audit runs in parallel with Sprint 3's multidim audit, triggered by the same `audit.run` Inngest event (corrected from audit/start — see D-05 v8.69); shared quota; per-audit cost budget rises to <US$3.50 to cover site-crawler cost. Clarified `technical-audit-run.ts` is fired by the SAME event as `run-audit.ts`. CLAUDE.md §2 carries the matching design note.
