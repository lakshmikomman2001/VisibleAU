...
1. Files named visibleau-prototype-vX.XX.jsx (e.g. v2.59) are OLD PHASE 1 prototype drafts.
...
1. The Phase 2 LLD is a single file (visibleau-7layer-lld.md); its version lives inside it
   (”# Version: 8.55”). No LLD draft files exist to confuse.
1. Ignore EVERY “-vN.NN” dated draft of any file (sprint prompts, prototypes, master-bundle
...
1. Read the Phase 1 sprint prompts, CLAUDE.md, Foundations, the CURRENT LLD (v8.55), and the
   Phase 2 prototype.
1. Pick a GENUINELY FRESH ANGLE each pass (don’t repeat a prior angle — the LLD changelog lists
   what’s already been checked).
1. Find conflicts. For EACH, FIRST assess which document is authoritative (LLD schema = canonical
...
1. VERSION-BUMP the LLD (v8.55 -> v8.56) with a detailed changelog entry: the conflict, the
...
1. VERIFY before presenting: 37 CREATE TABLEs, all 16 GAPs, serve() = 25/25 Inngest functions,
...
1. present_files the changed document(s). THEN Sri must download them and re-upload to the next
...
1. “0 engines / 0 LLM calls” (OPEN — pending). On an Agency-tier brand the progress page shows
...
1. Ask Sri what’s next (continue Phase 2 LLD/prototype conflict audits? apply the open
   fix-audit-zero-engines.md? begin Phase 2 sprint-prompt design? fix another live screen?).
1. Respond English then Telugu; hold the Performance / Security / Scalability / UX non-negotiables.