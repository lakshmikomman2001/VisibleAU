...
# FIX-AN Step 12b — new step: apply RLS policies to local PostgreSQL for multi-tenant DB isolation
# FIX-AO Step 12 — drizzle.config.ts Option A replaced with tablesFilter (Sprint 1 W1 canonical preserved)
# FIX-AP Step 12b — replaced psql heredoc with psql -f (heredoc fails on Windows CMD/PowerShell)
# FIX-AQ Step 13 — removed double closing ``` fence (markdown rendering bug)
# FIX-AR Step 12 — drizzle.config.ts update moved BEFORE generate command (was after — wrong order)
# FIX-AS Step 12b — scripts/db/rls.sql now idempotent (DROP POLICY IF EXISTS before each CREATE)
# FIX-AT Step 12b — added mkdir -p scripts/db before creating rls.sql (subdirectory may not exist)
# FIX-AU Checklist — added RLS policy verification to CLEAN STATE checklist
# FIX-AW Step 4  — set_config moved after orgRow SELECT; newOrgId may not exist if onConflictDoNothing fired
# FIX-AX Step 12b — sprint2-rls.sql: correct policy types (audits needs 2 policies; citations FOR SELECT; add llm_response_cache)
# FIX-AY Step 4  — stale FIX-AK comment updated: newOrgId is for INSERT id, not RLS context (set_config uses orgRow.id per FIX-AW)
# FIX-AZ Step 12 — verify block now checks app tables after migration; auth_* tables noted as first-boot-only
# FIX-BA Steps 4b,4b,11B,11E — headers() → await headers() (Next.js 15 made headers() async;
#         all 4 call sites: current-user.ts, sync-user route, layout.tsx, Step 11E example)
...
2. `sri-visibleau-foundations.md` v1.12 — read §2 (folder structure) + §3 (schema)
3. `sri-visibleau-sprint-1-prompt.md` — understand what Sprint 1 built
    ...
    organization({
      ...
      organizationCreation: {
        afterCreate: async ({ organization: org, member }) => {
          ...
          // Generate the UUID for the VisibleAU organizations row.
          // org.id is Better Auth's TEXT id — not suitable as our UUID primary key.
          // set_config uses orgRow.id AFTER the SELECT (see FIX-AW below), not this value.
          ...
          if (member?.userId) {
            // Find the org row by clerkOrgId — works whether the INSERT above
            // created a new row or onConflictDoNothing found an existing one.
            ...
            if (orgRow) {
              // FIX-AW: set_config AFTER finding orgRow so we always use orgRow.id (UUID).
              // Using newOrgId before this point is wrong if onConflictDoNothing fired
              // (newOrgId would be a fresh UUID that was never inserted).
              // Postgres superuser bypasses RLS anyway (FIX-AB) — this is belt-and-suspenders.
              await db.execute(
...
**First, update `drizzle.config.ts`** (FIX-AR: must do this BEFORE running generate):
Better Auth creates `auth_*` tables itself on first boot. Without `tablesFilter`,
`drizzle-kit generate` will try to CREATE those tables and fail with
"relation already exists" after Better Auth has already created them.

```typescript
import type { Config } from 'drizzle-kit';

export default {
  schema: './db/schema/index.ts',   // Sprint 1 W1 fix — barrel includes auth.ts
  out: './db/migrations',
  dialect: 'postgresql',
  dbCredentials: { url: process.env.DIRECT_URL! },
  // Exclude Better Auth tables — managed by Better Auth adapter, not drizzle-kit.
  tablesFilter: ['!auth_users', '!auth_sessions', '!auth_accounts',
                 '!auth_verifications', '!auth_organizations',
                 '!auth_members', '!auth_invitations'],
} satisfies Config;
```
...
# Generate migration files from the updated schema (auth_* tables excluded by tablesFilter)
...
Verify the **app** tables were created by migration:
```bash
psql -U postgres -d visibleau -c "\dt"
```

Expected (organizations, users, brands, drizzle internal):
```
organizations
users
brands
...
**Note on auth_* tables:** Better Auth creates `auth_users`, `auth_sessions` etc.
automatically the first time `pnpm dev` boots (via the drizzle adapter). They will
NOT exist yet after this migration step — that is correct and expected.
After running `pnpm dev` once, verify them with:
```bash
...
## STEP 12b — APPLY RLS POLICIES (MULTI-TENANT DB ISOLATION)

This step applies PostgreSQL Row-Level Security policies to the app tables.
Without these, tenant isolation relies only on the application layer — the
database-level security backstop is missing.

**What RLS does:** every query on `organizations`, `users`, and `brands` is
automatically filtered to the current org's rows. Even if a bug in app code
forgets an `organizationId` filter, Postgres blocks cross-org data leaks.

**Why it works with local postgres superuser:** the `postgres` superuser
bypasses RLS by default (FIX-AB). So locally your app still functions without
policies — but the policies must exist for production correctness and for the
cross-org acceptance test (`Second org user CANNOT access first org's brand`).

First create the `scripts/db/` directory and SQL file:

```bash
# Windows CMD:  mkdir scripts\db
# PowerShell:   New-Item -ItemType Directory -Force scripts\db
# bash/Mac:     mkdir -p scripts/db
```

Create `scripts/db/rls.sql`:
```sql
-- Idempotent: safe to re-run (DROP IF EXISTS before each CREATE)
-- Enable RLS on all tenant-scoped tables (ALTER TABLE ENABLE is idempotent)
...
Apply to **dev database** (works on Windows CMD, PowerShell, and bash):
```bash
psql -U postgres -d visibleau -f scripts/db/rls.sql
```

Apply to **test database**:
```bash
psql -U postgres -d visibleau_test -f scripts/db/rls.sql
```

Verify policies were created:
```bash
psql -U postgres -d visibleau -c "\dp organizations users brands"
```

You should see policies listed for all three tables.

**Sprint 2+ tables:** when Sprint 2 creates `audits`, `citations`, and
`llm_response_cache`, run `scripts/db/sprint2-rls.sql` after Sprint 2 migrations.
This matches Sprint 2 P2 canonical exactly:
```sql
...
-- audits: separate SELECT + write policies (Sprint 2 P2 canonical)
DROP POLICY IF EXISTS "Users see only their org's audits" ON audits;
CREATE POLICY "Users see only their org's audits"
  ON audits FOR SELECT
  USING (organization_id::text = current_setting('app.current_org_id', true));

DROP POLICY IF EXISTS "Users mutate only their org's audits" ON audits;
CREATE POLICY "Users mutate only their org's audits"
  ON audits FOR ALL
  USING (organization_id::text = current_setting('app.current_org_id', true))
  WITH CHECK (organization_id::text = current_setting('app.current_org_id', true));

-- citations: read-only via parent audit's org (Sprint 2 P2 canonical — FOR SELECT only)
DROP POLICY IF EXISTS "Users see only their org's citations" ON citations;
...
-- llm_response_cache: ENABLE RLS but NO policy (shared across orgs, keyed by prompt+model)
-- Inngest jobs use postgres superuser which bypasses RLS.
-- App routes never query this table directly — only LLMService via Inngest does.
```
Create `scripts/db/sprint2-rls.sql` with the above and run:
```bash
psql -U postgres -d visibleau -f scripts/db/sprint2-rls.sql
psql -U postgres -d visibleau_test -f scripts/db/sprint2-rls.sql
```
...
- ✅ RLS policies applied: `psql -U postgres -d visibleau -c "\dp organizations users brands"` shows policies
