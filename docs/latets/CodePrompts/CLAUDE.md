...
- **Sentry** — error monitoring. `@sentry/nextjs` with `withSentryConfig`. Three config files: `sentry.client.config.ts` (NEXT_PUBLIC_SENTRY_DSN + replay), `sentry.server.config.ts` (SENTRY_DSN + httpIntegration), `sentry.edge.config.ts` (minimal — middleware errors).

### Sprint 12 production patterns (JU1 — added for Claude Code context)

Claude Code must know these patterns when working in any Sprint 12 file:

- **PostHog App Router init:** `PHProvider` ('use client') wraps `<body>` in `app/layout.tsx`. `PostHogPageView` inside `<Suspense fallback={null}>` inside `PHProvider` captures route-change pageviews. `CookieConsentBanner` also inside PHProvider.
- **Cookie consent:** `components/domain/shared/cookie-consent-banner.tsx` — localStorage persistence, `posthog.opt_out_capturing()` on Decline, link to `/privacy#cookies`. Mounts in both marketing and auth layouts.
- **Sentry error capture:** `error.tsx` and `global-error.tsx` both use `Sentry.captureException(error)` in `useEffect([error])`. `global-error.tsx` must have `<html><body>` wrapper.
- **Public API routes:** `/api/health` (Supabase ping), `/api/badge` (SVG + Upstash rate limit + service role key), `/api/demo` (NODE_ENV production guard). All three in `middleware.ts` publicRoutes.
- **Data retention:** `inngest/functions/audit-data-retention.ts` — weekly Sunday 04:00 UTC. Deletes citations then audits older than 12 months. Separate step for churned org cleanup at 13 months.
- **Demo data:** `DEMO_MODE=true` + `NODE_ENV !== 'production'` both required. Seeded via `scripts/seed-demo-data.ts` (3 workspaces: tradies, allied-health, saas).
