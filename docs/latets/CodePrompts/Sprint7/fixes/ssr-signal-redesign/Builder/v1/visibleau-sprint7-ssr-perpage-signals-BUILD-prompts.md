...
> Carries the **Signals /6** dimension. No prototype exists — build on the design system, reusing the
> Negative Signals + Prompt Injection sections being removed from the SSR page (relocate, don't
> rebuild). **Data source (critical):** read `findings.content.negativeSignals` and
