...
6. `visibleau-prototype-SignalsAudit-component.jsx`          — NEW prototype component for the Signals page (the SSR page reuses the existing `SsrCheck`; the Signals page had none, so this was created). To be folded into `prototype.jsx`.
...
- [ ] Signals page matches the **`SignalsAudit`** prototype (item 6): same PageShell/Card/Badge system, score block, two sections, `/6` red at 0/6. (The SSR page reuses the existing `SsrCheck`; only the Signals page needed a new prototype.)
