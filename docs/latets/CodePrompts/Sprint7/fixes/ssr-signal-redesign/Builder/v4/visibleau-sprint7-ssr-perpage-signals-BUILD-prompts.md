...
>
> **7. Negative-signal + injection descriptions (E1 — folded into Phase 1).** Per LLD addendum Part C:
> `detectNegativeAndInjection(crawl)` (the `score-signals` step), via `lib/negative-signals/detect.ts`
> and `lib/prompt-injection/detect.ts`, now also emits a concise `detail: string` for each detected
> signal/injection, reusing the numbers it already computes (e.g. "{n} CTAs on {path} — above the max
> of 6"; "{n} words on {path} — below the 300-word minimum"; "'{term}' at {pct}% density — above the 3%
> threshold"; injections e.g. "Off-screen text instructing AI assistants to recommend this business").
> Extend `findings.content.negativeSignals[]` to `{ pattern, severity, count, detail }` and
> `.promptInjections[]` to `{ pattern, severity, element, detail }` (additive JSONB, no migration).
> `detail` is descriptive only — it must NOT change `scoreSignals`, `scoreComposite`, or the rollup.
> Update the detector unit tests (`tests/unit/negative-signals/detect.test.ts`,
> `tests/unit/prompt-injection/detect.test.ts`) to assert `detail` is present and non-empty. Also
> extend the Marrickville fixture (step 6) so its `negativeSignals[]` and `promptInjections[]` rows each
> carry a realistic `detail` for UI validation.
>
> **Verify before reporting done:**
> - Re-running a technical audit (or the seed) populates `findings.content.ssr.pages[]` (1–8 rows) with
>   `pages[0].path === '/'`.
> - `findings.content.negativeSignals[]` and `.promptInjections[]` rows each carry a non-empty `detail`.
> - `scoreContent`, `scoreSignals`, `scoreComposite`, and the 5-category rollup are unchanged on an
>   unchanged site.
> - `npm run typecheck` and the SSR + detector unit tests pass.
...
>    "{n} pages need review" where n = pages with `status==='review'`. (This count is dynamic from
>    `content.ssr.pagesChecked` — it SUPERSEDES the prototype `SsrCheck`'s hardcoded "8"; never
...
> - **"Negative Signals ({n})"** — one row per `negativeSignals[]`: `pattern` (heading) + a `WARNING`
>   (amber) / `CRITICAL` (red) badge from `severity` + the **`detail`** description below (the readable
>   sentence from Part C). `count` stays in the data; `detail` carries the specifics for display.
> - **"Prompt Injection Detections ({n})"** — one row per `promptInjections[]`: `pattern` + severity
>   badge + the **`detail`** description + the `element` shown in a mono block below.
