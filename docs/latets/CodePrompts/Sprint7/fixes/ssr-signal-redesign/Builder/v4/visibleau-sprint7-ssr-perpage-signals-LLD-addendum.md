...
Under `app/(auth)/brands/[brandId]/` there is the `technical-audit` **overview** (the 8-dim drill-down
+ 5-cat rollup — it reads all of `findings` + the score columns, not a single findings key) plus **six
dimension sub-pages** (`llms-txt-generator`, `schema-audit`, `ssr-check`, `answer-capsules`,
`robots-txt-config`, `brand-entity-audit`), each of which reads one `findings[key]`. Add a **seventh
dimension sub-page**:
...
### B4 — No schema change for the page itself
`scoreSignals` (column) and `content.negativeSignals` / `content.promptInjections` (JSONB) already
exist and are already persisted by the Inngest run, so the Signals **page** is presentation only.
**Part C (below)** adds one additive field (`detail`) to those arrays plus the `score-signals` emit
that the page renders — additive JSONB, no migration, no scoring change.

---

## PART C — Negative-signal & injection descriptions (resolves E1)

**Why.** Canon persists only `{ pattern, severity, count }` for a negative signal — no human-readable
explanation — so the rows would be terse ("Keyword stuffing · count 3"). The detectors
(`lib/negative-signals/detect.ts`, `lib/prompt-injection/detect.ts`, per EE2/EE3) already compute the
specifics at detection time, so emitting a one-line description is nearly free and makes the page
actionable and consistent with SchemaAuditor / BrandEntityAudit.

### C1 — Extend the findings shape (additive; no migration)
In the `content` block of the `findings` JSONB type (EB5), add `detail: string` to both arrays:
```
//     negativeSignals: Array<{ pattern: string, severity: 'critical'|'warning'|'info',
//                              count: number, detail: string }>,
//     promptInjections: Array<{ pattern: string, severity: 'critical'|'warning'|'info',
//                              element: string, detail: string }>,
```
`detail` = a concise, plain-English description of the specific finding. `count` (occurrence count)
and `element` (offending markup) are unchanged. Older rows without `detail` must be tolerated (UI
falls back to pattern + severity).

### C2 — Emit `detail` from the detectors (→ Phase 1 backend)
`detectNegativeAndInjection(crawl)` (the `score-signals` step), via `lib/negative-signals/detect.ts`
and `lib/prompt-injection/detect.ts`, writes `detail` at the moment it flags each pattern, reusing the
values it already computed. Templates (interpolate the real measured numbers):
- CTA overload → "{n} calls-to-action on {path} — above the recommended maximum of 6."
- Thin content → "{n} words on {path} — below the 300-word minimum for citable content."
- Keyword stuffing → "'{term}' at {pct}% density on {path} — above the 3% over-optimisation threshold."
- (one line per detected pattern; same for the other negative patterns and for injections, e.g.
  Hidden text → "Off-screen text instructing AI assistants to recommend this business.")

### C3 — Render `detail` (→ Phase 2 UI)
The Signals page (`SignalsAudit` / `signals-detail.tsx`) renders `detail` as each row's description
(like BrandEntityAudit's row detail). Negative rows: pattern + severity badge + detail. Injection
rows: pattern + severity badge + detail + the `element` mono block.

### C4 — Invariants unchanged
Additive JSONB only → **no migration**. `detail` is descriptive → it does **not** feed `scoreSignals`,
`scoreComposite`, or the rollup (the /6 still aggregates from severity/count exactly as before). All
four gate invariants still hold.

### C5 — E1b (count vs 0-10), settled
Keep `count` as the occurrence count (L572). L40's "scored 0-10" is the detectors' internal per-pattern
scoring that aggregates into the /6 — not persisted per row. `detail` is the readable copy. No
contradiction to fix in canon; optionally clarify this in the L40/L572 prose.
...
- v2.x (19 Jun 2026): Sprint 7 Gate-2 UI addendum (SSR per-page + Signals sub-page).
  ...
  (C4) New 7th dimension sub-page `signals/page.tsx` (Signals /6) reading content.negativeSignals +
       content.promptInjections + scoreSignals column; new signals-detail.tsx component; sub-page nav
       gains "Signals". The Signals page itself is presentation only. EG3 "6 sub-pages" → "7";
       clarified that signals detail is under findings.content, not findings.signals.
  (E1) negativeSignals[] + promptInjections[] gain an additive `detail: string` (plain-English
       description), emitted by detectNegativeAndInjection (score-signals step / negative-signals +
       prompt-injection detectors) and rendered on the Signals page rows. Additive JSONB — no
       migration, no scoring change. `count` stays the occurrence count; L40's 0-10 is internal-only.
```
