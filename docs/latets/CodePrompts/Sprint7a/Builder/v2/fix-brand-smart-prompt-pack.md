...
# Version: 1.1 | Post-Sprint 7 (updated: vertical pack UI reconciliation added)
...
- Audit Running component / step-label source — update step 2 label (Phase 5b.1)
- Vertical Pack Browser query/API — update "active brands" count logic (Phase 5b.2)
- Vertical Pack Browser component — update count label + add tooltip (Phase 5b.3–4)
...
Brand prompt packs = RUNTIME PROMPT SOURCE (10 prompts per audit, brand-specific, AI-derived)
The two systems are complementary: `lib/prompts/templates.ts` draws from the same categories
as the vertical packs; brand classification selects the right category and generates a
brand-specific subset. The vertical pack UI surfaces the shared library; the brand prompt
pack is the per-brand runtime selection from it.
