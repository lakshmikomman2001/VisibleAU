...
# Version: 1.1 (updated: vertical pack UI reconciliation added as Phase 5b)
# Date: June 2026
...
### The second conflict discovered (post-Sprint 5 review)

After the fix was designed, a screenshot of the live Sprint 5 Vertical Pack Browser
was reviewed. It showed the built app correctly matches the prototype — AU Tradies v1.0
(124 prompts, 1 active brand), AU Allied Health v1.0 (104 prompts, 0 active brands),
AU SaaS v1.0 (108 prompts, 2 active brands), Professional Services and Real Estate locked
at V1.1, Hospitality / Retail / Beauty as Coming Soon.

This surfaced a UI conflict: the two prompt systems (vertical packs + brand-specific packs)
were not reconciled. Specifically:

1. The Audit Running screen step 2 says "Generating prompts (10 from vertical pack)" —
   wrong once brand-specific packs are active.
2. The Vertical Pack Browser "N active brands" count includes all brands that selected
   that vertical, even those now running brand-specific prompts — misleading.
3. No UI copy explained the relationship between the two systems to operators.

Phase 5b of the fix prompt addresses all three. The fix prompt was updated to v1.1
and this handoff to v1.1 to reflect the addition.
...
### F6 — Phase 5b specific checks (vertical pack UI reconciliation)
- [ ] Step 5b.1 — Audit Running label: confirm the label source location is found during
      Phase 1 investigation BEFORE writing any code. It could be in the Inngest function
      step metadata, a React component fixture, a server action, or an API response. The
      fix must go in the right place — not a duplicate string somewhere else.
- [ ] Step 5b.2 — Active brands count query: confirm the Drizzle query correctly uses
      `isNull(brands.promptPack)` as the primary filter condition. A brand with
      classification_status='complete' but no prompt_pack stored (edge case: failed
      buildPromptPack) should still count as "using pack" — the null check handles this.
- [ ] Step 5b.3 — Label copy: "N brands using pack" is the approved wording. Do not
      change it to anything else without flagging.
- [ ] Step 5b.4 — Tooltip: confirm the tooltip text is present but not intrusive. It
      should appear on hover only — not always visible. One sentence maximum.
- [ ] Verify that the vertical pack prompt library editor (PromptLibraryEditor screen)
      is unchanged — it shows the shared pack prompts and is not affected by brand-
      specific classification. Its "Read-only in v1" badge and content remain as-is.
...
### Companion file (required reading for review):
  fix-brand-smart-prompt-pack.md  ← the actual Claude Code prompt to review (v1.1)
...
### Existing files Claude Code will modify (additive only):
  ...
  Vertical Pack Browser query/API             ← update "active brands" count (Phase 5b.2)
  Vertical Pack Browser component             ← update label + tooltip (Phase 5b.3–4)
  Vertical Pack Detail component              ← update matching stat card (Phase 5b.3)

### Zero changes to:
  Any existing schema columns · scorer.ts · citation-detector.ts · LLM impl files ·
  Any existing Inngest functions · Vertical pack prompt data files · API response shapes ·
  Existing brands or audit history · The brand wizard vertical selector UI ·
  The prompt library editor content (remains Read-only in v1)

### Architecture relationship (locked — do not redesign):
  Vertical packs   = UPSTREAM TEMPLATE LIBRARY
                     Shared pool, operator-curated, 100+ prompts per pack,
                     browsable in the Vertical Pack Browser UI.
                     `lib/prompts/templates.ts` maps to the same categories.

  Brand prompt pack = RUNTIME PROMPT SOURCE
                      10 prompts per audit, brand-specific, AI-derived from
                      classification. Stored as JSONB on brands.prompt_pack.
                      The audit runner uses this — not the vertical pack directly.

  Flow: User picks vertical (Tradies/SaaS/Allied Health) → stored as hint on brand →
        Haiku classifies brand into specific category (e.g. saas_design_tools) →
        buildPromptPack() draws from templates.ts for that category → stored on brand →
        audit runner reads brand.prompt_pack → runs audit.

  The vertical pack browser UI remains accurate as the shared library view. The "N brands
  using pack" count shows which brands have not yet graduated to brand-specific prompts.
