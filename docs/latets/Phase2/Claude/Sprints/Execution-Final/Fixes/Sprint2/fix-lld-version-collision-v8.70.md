# Claude Code — METADATA FIX (docs only): version-number collision — bump audit/start canon correction to v8.69
...
**The audit/start canon correction must be v8.69.** This is a documentation-only metadata fix — no source, no
...
2. **Re-label the audit/start canon-correction CHANGELOG entry** from v8.69 → **v8.69**:
...
- Docs only. No source changes. No spec content changes — only the version label/stamps on the canon-correction
  change set move from v8.69 → v8.69.
- v8.69 must remain EXCLUSIVELY the TC-01 task-creation fix. v8.69 must be EXCLUSIVELY the audit/start canon
...
2. The CHANGELOG now has, top-down: **v8.69** (audit/start canon correction, four functions listed) ABOVE
...
3. `grep -rn "v8.69" phase-2/01-lld phase-1/02-lld` → remaining v8.69 references are ONLY the TC-01 entry and any
   legitimate historical mentions; the audit/start correction stamps now read v8.69.
4. The v8.69 (TC-01) entry text is unchanged.

## REPORT
- Confirmed header version before (8.69) and after (8.70) on both LLDs.
- Confirmation that v8.69 = TC-01 only and v8.69 = audit/start canon correction only (no overlap).
- The inline stamps updated to v8.69.
...
fix, current canonical is **v8.69**. Update the handover/notes so the next session knows:
- **v8.69** = current canonical (audit/start → audit.run canon correction)
