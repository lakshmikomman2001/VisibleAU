...
# Version: 1.4 | Built against: LLD v8.68 (REVIEWED) | Sprint: 4 of 9 | 4 weeks
...
are wired in by the sprints that create their tables (S5/S6) — the section framework is built now. (LLD 8939–8945.)
...
- **GAP coverage:** none new; this sprint *surfaces* GAP outputs (fan-out, topical, archetype
  now; LinkedIn/consensus/entity-home/knowledge-panel section slots are wired in by S5/S6).
...
`reports_brand_type_idx (brand_id, report_type, created_at DESC)`. **DDL note:** the LLD's
generated_reports block has a stray-comma typo around created_at/updated_at (missing comma
after `created_at`, trailing comma after `updated_at`) — emit CLEAN, valid SQL / Drizzle here;
do not reproduce the typo.
...
11. Entity Home section when entity_home_same_as_count < 3 (**content_structure_audits — S6;
    omitted until then**).
LLM call: `selectModel(tier, engine, 'narrative_generation')` (§0.4). **Section framework vs
forward reads (build-order, important):** the narrative-generator builds the SECTION FRAMEWORK
now — the rendering logic for all 12 section types and the include/order handling. For the
sections whose source tables exist this sprint (executive_summary, score_breakdown,
mention_source_divide, fan_out_coverage, topical_gap_summary — from S3 + Phase 1 data), wire
the reads now. For the sections whose tables are created LATER —
linkedin_performance/consensus_score/knowledge_panel_status (S5), source_type_gaps
(citation_source_intelligence, S5), evidence_snapshots (evidence_snapshots table, S5 —
Agency+ per RULE 8), entity_home_status (content_structure_audits, S6), and agent_readiness
(agent_readiness_scores, S6) — **do NOT import or query those tables in
S4** (they don't exist yet → TypeScript/SQL would fail). That accounts for all 12 section
types: 5 wired now, 7 as forward slots. Instead the framework exposes a typed
slot per section; the sprint that creates each table wires its read into that slot (the same
way §6.4's alert-composer has its hallucination/consensus triggers wired in S5). Each section
still renders only when its data is present — matching the LLD's RULE 7/10/11 "included when a
row exists" — but activation happens in the owning sprint, NOT magically with zero code. (If
you prefer a single-pass alternative: a `to_regclass('<table>') IS NOT NULL` raw-SQL existence
guard per forward read makes the function safe to run in the S4→S6 window and genuinely
self-activating; either approach is acceptable, but the table reads cannot be written naively
in S4.)
...
- **Model (MS-02):** `selectModel(tier, engine, 'narrative_generation')` — cheapest model;
  never hardcoded.
- **Reads — split by build-order (§6.1):** wire NOW (tables exist this sprint): visibility_trends,
  query_fan_out_results, topical_coverage_gaps (S3 + Phase 1). Forward slots WIRED IN THE OWNING
  SPRINT (do NOT import/query in S4 — the tables don't exist yet): citation_source_intelligence
  (S5), linkedin_presence_audits (S5), brand_consensus_checks (S5), brand_entity_scores S5 cols,
  evidence_snapshots (S5, Agency+), content_structure_audits (S6), agent_readiness_scores (S6).
  Applies narrative-generator RULES 1–11; each section renders only
...
>    need score_delta>0 AND sample_quality>='Likely'; the generator builds the SECTION
>    FRAMEWORK for all 12 section types and wires the reads whose tables exist in S4
>    (visibility_trends, query_fan_out_results, topical_coverage_gaps); the S5/S6 sections
>    (linkedin_performance/consensus/knowledge_panel/entity_home/source_type_gaps) are
>    unwired typed slots — do NOT import or query those tables in S4, they don't exist yet;
>    each is wired by the sprint that creates its table); pdf-builder
...
- `narrative-generator.test.ts` — RULE 1 (no causal language at quality_status='insufficient');
  RULE 3 (key win needs score_delta>0 AND sample_quality>='Likely'); a section whose table EXISTS
  in S4 (e.g. fan_out_coverage) is omitted when that table has no row for the period; the
  forward-slot sections (linkedin_performance etc.) render nothing in S4 because their slots are
  unwired (assert the framework omits them, NOT that an absent table returns empty);
...
- **Querying the S5/S6 tables in S4.** linkedin_presence_audits / brand_consensus_checks /
  brand_entity_scores cols (S5) and content_structure_audits (S6) do NOT exist at S4 build
  time — do not import or SELECT them here. Build the section framework with unwired forward
  slots; the owning sprint wires each read. (A `to_regclass` existence guard is the only way
  to touch them in one pass.)
...
alert-composer + narrative-generator (built with the section framework + unwired forward
...
- v1.2 — Gate 2 pass-2 finding applied (Phase-1-reuse-integrity angle), validated first.
  ...
  types — added the two missing ones to the forward-slot list: evidence_snapshots (S5, Agency+
  per RULE 8) and agent_readiness (agent_readiness_scores, S6); reconciled §8.1's forward read
  list to match. Now all 12 types are explicitly bucketed (5 wired now, 7 forward slots). No
  LLD change. Pass-2 verified both v1.1 fixes landed correctly (the S4-01 reframe consistent
  across 7 locations, the §6.4 contradiction resolved, the §11 test corrected) and confirmed all
  8 Phase-1 "reuse, don't re-create" claims are LLD-backed with correct sprint attribution
  (lib/pdf/theme.ts S9, lib/email/client.ts S2, drift/detected S8, send-weekly-digest, etc.).
- v1.1 — Gate 2 findings applied (reviewer chat), validated against canon first. S4-01 [MOD]:
  corrected the unimplementable "self-activate with zero code change" claim — at S4 build time
  the S5/S6 source tables don't exist (linkedin_presence_audits is S5, content_structure_audits
  is S6), so they can't be imported/queried. Reframed §6.1/§8.1/§10/§14/§13 as: S4 builds the
  SECTION FRAMEWORK + wires the reads whose tables exist now (S3/Phase 1); the
  linkedin/consensus/knowledge-panel/entity-home/source-type-gaps sections are unwired typed
  slots that the OWNING sprint (S5/S6) wires in (matching the §6.4 alert-composer framing); a
  to_regclass existence guard offered as the single-pass alternative. Fixed the §11 test (the
  linkedin "no row" assertion couldn't run against a missing table → now asserts the framework
  omits unwired slots and tests omission on a table that exists in S4). S4-02 [LOW]: added a
  "emit clean DDL" note to §5.2 (the LLD generated_reports block has a stray-comma typo —
  flagged to Sri as an LLD cleanup, not reproduced here). Reviewer confirmed the four high-risk
  areas clean (structural traps C2, narrative rules C3, event chain + reuse C4, EM-01 dedup C5)
  and that the recurring §1-under-enumeration pattern did NOT recur.
