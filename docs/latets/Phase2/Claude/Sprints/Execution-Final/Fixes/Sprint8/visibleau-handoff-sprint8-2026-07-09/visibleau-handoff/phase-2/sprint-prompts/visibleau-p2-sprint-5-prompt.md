...
# Version: 1.5 | Built against: LLD v8.68 (REVIEWED) | Sprint: 5 of 9 | 4 weeks
...
- Wire the **S4 alert-composer** triggers: detect-hallucinations → the hallucination alert;
  check-cross-platform-consensus → the consensus EMAIL alert at `consistency_score < 60` (LLD
  8402 — distinct from the in-app Action Center consensus alert at < 70, LLD 7252). (Drift +
  volatility were already wired in S4 from Phase 1 / S3.)
...
- **THE EXPLAINABILITY CONTRACT (platform-wide, GAP 9 / v8.33 — NON-NEGOTIABLE):** every
  score-bearing Phase 2 API response MUST include the contract fields alongside its score data:
  `rationale` (plain-English WHY for a non-technical agency owner), `confidence_label`
  ('High'|'Medium'|'Low'|null), `confidence_note` (null for High; a caveat for Medium/Low),
  `top_action` (single next step or null). Routes populate these via
  **`ExplainabilityService.annotate()` (lib/platform/explainability.ts — CREATED in S3, the first
  scored sprint; S5 REUSES it, "if not already present")** before responding; an empty `rationale`
  FAILS CI (`rationale.length > 30`). This applies to every scored S5 route in §9 — the trust
  summary, entity score (score_of_10), the read-time hallucination risk, consensus, LinkedIn,
  YouTube — which are exactly the trust numbers customers act on, so they must carry the
  Where/Why/What-next/Confidence annotation, not ship as raw scores.
...
└── index.ts
lib/platform/
└── explainability.ts                // §0.4 ExplainabilityService.annotate() — REUSE (created in S3); import + wire into scored routes
...
components/domain/trust/   (the 13 components listed in §6U)
...
linkedin-presence-scorecard.tsx (score /100 + company + founder breakdown) + linkedin-gap-row.tsx
...
`hallucination/detected`** (`{ organizationId, brandId, incidentId }`) for **critical +
warning** only (the two non-info severities — WH-01b; fanout-webhooks maps to external
`hallucination.detected`; non-info only, to avoid alert fatigue). NOTE: the LLD's WH-01b
wording says `severity='critical'|'high'`, but the severity enum is critical|warning|info (no
'high') — use critical+warning, matching the email-alert path in this same function.
...
(brand_id, source_type)** (CU-01). TWO DISTINCT consensus alerts at TWO thresholds: the in-app
**Action Center** alert fires at `consistency_score < 70` (LLD 7252); the **S4 alert-composer
EMAIL** alert fires at `consistency_score < 60` (LLD 8402). Wire both at their own threshold —
...
Every route: Better Auth session + org scoping; Zod; correct codes; tier gates. **Every
score-bearing route (trust, entity-score, hallucinations risk, citation-sources,
linkedin-presence, consensus-score, youtube-presence) passes its data through
`ExplainabilityService.annotate()` (REUSE the S3-created service) before responding** — the
response carries `rationale` + `confidence_label` + `confidence_note` + `top_action` alongside
the scores (the platform contract, §0.4; empty rationale fails CI).
...
>    from §6.5. All LLM calls use selectModel(tier, engine, …) — no hardcoded models. PLUS
>    (G3-01) the platform-wide explainability contract: REUSE `lib/platform/explainability.ts`
>    (ExplainabilityService.annotate(), created in S3 — do NOT redefine it) and wire annotate()
>    into every scored S5 route (/trust, /entity-score, etc.) so the response carries rationale +
>    confidence_label + confidence_note + top_action before returning (empty rationale fails CI).
...
>    dormant.
> 6. The 8 Trust screens (§6U) + the 13 components: the read-time Hallucination Risk card
...
- `trust-explainability.test.ts` — the scored S5 routes (/trust, /entity-score, etc.) pass their
  data through ExplainabilityService.annotate() and the responses include rationale (non-empty,
  >30 chars), confidence_label, confidence_note, top_action alongside the scores; the service is
  imported, not redefined (reused from S3). (Guards G3-01 — the platform contract.)
...
```bash
# G3-01: scored S5 routes annotate via the REUSED service (created in S3, not redefined here)
...
test ! -f lib/platform/explainability.ts && echo "OK: reused from S3 (not recreated here)" || grep -Rc "class ExplainabilityService\|export.*annotate" lib/platform/explainability.ts  # service exists (from S3)
...
# CT-04: no risk column; risk is read-time. Target a COLUMN declaration, not comment text
# (the schema legitimately carries the CT-04 "NO risk column / risk = LEAST(…)" comment):
grep -iE "^\s*risk[_a-zA-Z]*\s*:" db/schema/hallucination-incidents.ts || echo "no risk col OK"   # → prints "no risk col OK"
...
## CHANGELOG
- v1.5 — Re-pinned to canon v8.68 (consolidated LLD+prototype hygiene pass: S7b-02
...
- v1.4 — Gate 3 cross-prompt audit fix applied (G3-01), validated against canon first. Same
  finding as S3: the platform-wide explainability contract (GAP 9 / v8.33; LLD 5537/5567; empty
  rationale fails CI) was referenced 0× in S5 despite S5 shipping the heaviest scored routes in
  the series (/trust, /entity-score [score_of_10], hallucination risk, consensus, linkedin,
  youtube). Fixed: S5 now REUSES the ExplainabilityService that S3 (the first scored sprint)
  creates — imports lib/platform/explainability.ts (does NOT redefine it) and wires annotate()
  into every scored S5 route so responses carry rationale + confidence_label + confidence_note +
  top_action (§0.4 contract + §4 tree + §9 routes + §10 step 3 + a §11 test + a §12 grep asserting
...
- v1.2 — Gate 2 pass-2 finding applied (Inngest event-graph + grep-executability angle),
  ...
  case-insensitive) false-failed because it matched the mandated CT-04 comment text ("NO risk
  column", "risk = LEAST(…)") that the schema legitimately carries — tightened it to target a
  COLUMN declaration (`^\s*risk[_a-zA-Z]*\s*:`) so a correct schema passes. Verification-precision
  only; no schema/build change. Pass-2 also traced the full cross-sprint Inngest producer→consumer
  graph and found NO wiring conflicts: every S5 subscription has a real producer (audit/complete=
  Phase 1, citations/classified=S3, technical-audit/complete=S7 forward with the dual-emit note),
  the one emit (hallucination/detected) has a forward consumer (S8 fanout-webhooks), and the
  slash/dot convention is consistent across every edge. All three v1.1 fixes reconfirmed.
- v1.1 — Gate 2 findings applied (reviewer chat), all validated against canon first. S5-01
  [LOW-MOD]: the two consensus-alert thresholds were conflated — the in-app Action Center alert
  fires at consistency_score < 70 (LLD 7252), the S4 alert-composer EMAIL alert at < 60 (LLD
  8402). Fixed §0.2 + §8.6 to state both separately and dropped the "< 60/70" shorthand. S5-02
  [LOW]: the webhook emit filtered on a non-existent 'high' severity (the enum is critical|
  warning|info) — corrected §8.1 to critical+warning (the two non-info severities), matching the
  email-alert path in the same function; the LLD's WH-01b wording ('critical'|'high' at LLD 7161/
  7163) is flagged to Sri as an LLD-hygiene cleanup. S5-03 [LOW]: the §4 tree + §10 said "14
  components" but §6U specifies 13 (verified against the LLD's component list, which is also 13 —
  no missing component, the count was just off); corrected both to 13 and added the .tsx
  extension to linkedin-gap-row. No structural/feature change; no existing wiring touched.
  Reviewer confirmed the three high-risk areas clean: the read-time/structural traps (C2), the
  Inngest event taxonomy (C4), and — the point of the sprint — the S4→S5 wiring contract (C5,
  5 slots wired + 2 correctly deferred to S6). §1-vs-tree enumeration discipline held.
