# Claude Code — Sprint 1 Test Task Runner (v3)
...
3. `C:\startup\VisibleAU\src\docs\latets\CodePrompts\sri-visibleau-sprint-1-prompt.md`   — what Sprint 1 built + acceptance criteria
4. `Lucky_test_prompts.xlsx`            — confirm task 20 column C is blank (your only task)
5. `scripts/qa/sprint1/`               — read ALL batch scripts created in task 19
...
3. Read ALL scripts in `scripts/qa/sprint1/` — know what each one tests
