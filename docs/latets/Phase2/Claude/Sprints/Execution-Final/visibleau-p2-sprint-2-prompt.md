...
`'audit.run'` + `{ auditId }` (corrected from audit/start — see D-05 v8.69) — event+sleep, NOT a cron (TR-01). **QUOTA GATE (U-14):** before firing
`'audit.run'`, call `checkQuota(orgId, brandId)`; if not allowed, **do not fire** — call
...
`'audit.run'` + `{ auditId }` (corrected from audit/start — see D-05 v8.69), call `checkQuota(orgId, brandId)`** (same gate as Phase 1 audit-schedules
cron) — Phase 2 runs are additional to the scheduled cadence, never outside the quota gate.
LinkedIn/consensus checks are NOT quota-tracked (only `'audit.run'` consumes the slot).
...
- `validation-reaudit.integration.test.ts` — task→complete emits 'task/completed';
  over-quota path calls markReauditDeferred and leaves score_after NULL (no 'audit.run').
...
- **Firing 'audit.run' without checkQuota** (corrected from audit/start — see D-05 v8.69) in either Inngest function — system-triggered
