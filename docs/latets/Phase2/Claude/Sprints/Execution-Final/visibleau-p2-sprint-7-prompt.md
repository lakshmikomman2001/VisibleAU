...
# Version: 1.2 | Built against: LLD v8.67 (REVIEWED-r2) | Sprint: 7 of 9 | 4 weeks
...
- **OBLIGATION 1 — technical-audit-run.ts dual-emit (RE-01/SR-01/AE-01).**
  ...
  `audit.run` + `{ auditId }` (corrected from audit/start — see D-05 v8.69); it already scores brand_entity_scores AU signals). It currently emits only the
...
- 4 lib modules (§6): journey-runner, journey-scorer, comparison-runner, intent-classifier
  (+ types.ts, the JourneyTurn interface §6.0).
...
rows, the S3 benchmark shows the real comparison data (see §14). **READ THE LATEST ONLY (the
table accumulates per-audit):** comparison_prompt_results gets a fresh row-set on every
audit/complete, so the benchmark must show the CURRENT head-to-head, not every historical cycle.
Select the latest comparison per competitor×engine — `DISTINCT ON (competitor_domain, engine) …
ORDER BY competitor_domain, engine, run_at DESC` (or scope to the most recent audit_id). The
`comparison_competitor_idx (brand_id, competitor_domain, run_at DESC)` supports this. (This also
keeps any retry-driven duplicate rows from showing as separate verdicts.)
...
- **Trigger:** listens on the internal event **`journey/run-requested`** (slash convention),
  which the `POST …/journeys/[journeyId]/run` route emits via `inngest.send({ name:
  'journey/run-requested', data: { journeyId, brandId, organizationId } })`. (If a scheduled
  re-run is added later it can emit the same event from a cron; this sprint only wires the
  API-triggered path.) **Agency+
...
- Writes journey_run_results (one row per engine per run) with the journey-scorer output.
  **The result-row INSERT is ALSO a stable step (retry-idempotency):** wrap the per-engine
  write in `await step.run(\`persist-${engine}\`, async () => { /* INSERT journey_run_results */
  })`. journey_run_results is append-only with NO UNIQUE (correct — it's score history), so on
  an Inngest retry the memoized turn steps are skipped and the code after them re-executes; if
  the INSERT is not itself a step it would double-insert a spurious extra run into the scored
  series. Stepping the write makes the whole function — not just the LLM calls — retry-safe.
...
`audit.run` + `{ auditId }` (corrected from audit/start — see D-05 v8.69); scores brand_entity_scores AU signals). **On completion it must emit BOTH:**
```ts
...
- `POST …/journeys/[journeyId]/run` — execute a journey (Agency+); emits `journey/run-requested`
  (slash) which run-journey (§8.1) listens on.
...
# S7b-01: the per-engine result-row INSERT is itself a step (retry-idempotent)
grep -Rc "persist-" inngest/functions/run-journey.ts                                   # → ≥1
# run-journey trigger event named + agreed between producer (run route) and consumer
grep -Rc "'journey/run-requested'" app/api/brands/\[id\]/journeys/\[journeyId\]/run/route.ts inngest/functions/run-journey.ts  # → ≥2
...
## CHANGELOG
- v1.2 — Gate 2 pass-2 findings applied (retry-idempotency / read-consistency angle), all
  validated against canon first. S7b-01 [LOW-MOD]: run-journey stepped the per-turn LLM calls
  (SN-01) but the per-engine journey_run_results INSERT was not stepped — under Inngest's replay,
  a retry skips the memoized turn steps then re-executes the code after them, double-inserting a
  spurious run into the append-only (no-UNIQUE) score history. Fixed §8.1 to wrap the write in a
  stable `persist-${engine}` step + added a §12 grep. S7b-03 [LOW]: comparison_prompt_results
  accumulates per-audit, but §6U.4 didn't specify the S3 benchmark reads the LATEST comparison —
  an unfiltered read would show every historical cycle (and any retry duplicates). Specified
  DISTINCT ON (competitor_domain, engine) … run_at DESC (or scope to the latest audit_id), which
  the comparison_competitor_idx supports; this makes S7's own C4iii deliverable ("the S3 benchmark
  goes live") actually show the current head-to-head. S7b-02 [LOW-MOD] — ESCALATED to Sri (LLD-
  level, NOT applied to the prompt): run-comparison-prompts has no step structure, while run-journey
  got SN-01 — so an Inngest retry re-runs every comparison LLM call from scratch (cost waste) and
  re-inserts rows. The prompt faithfully mirrors the LLD's omission; recommend the LLD add a
  parallel step structure (step.run(`cmp-${competitor}-${engine}`, …) with persist-in-step) so
  run-comparison's retry-safety matches run-journey's. With your go-ahead the prompt can then
  require it. No structural/feature change; no existing wiring touched. Pass-2 confirmed the S7-01
  fix correct and the write semantics themselves sound (both tables append per the established
  audit/complete-listener pattern); the gaps were retry-idempotency + read-filtering, not table design.
- v1.1 — Gate 2 finding applied (reviewer chat), validated against canon first. S7-01 [LOW]:
  run-journey's trigger was described as "event-driven" but the event was unnamed (the LLD
  leaves it unnamed too, unlike every other Inngest fn in the sprint), so Claude Code would have
  had to invent it. Named it `journey/run-requested` (slash internal-chaining convention) — the
  POST …/journeys/[journeyId]/run route emits it, run-journey listens on it; wired in §8.1 + §9
  and added a §12 producer↔consumer grep. Clarified the scheduled-re-run path (a later cron can
  emit the same event; this sprint wires only the API-triggered path). Low-risk (both ends are
  built in this sprint — no cross-sprint mismatch), but worth the precision. Also adopted the
  reviewer's optional §1 polish (noted types.ts) — no under-build risk, the §1-vs-tree pattern
  did NOT recur. No structural/feature change; no existing wiring touched. Reviewer confirmed the
  prompt the cleanest in the series: all three cross-sprint contracts correct (the
  technical-audit-run DUAL-EMIT C4i — the #1 risk; crawler reuse C4ii; the S3 benchmark
  completion C4iii), all five structural traps (C2), the journey_score formula (C3), the SN-01
  step structure (C5), and the v8.19 tier gating (C6).
