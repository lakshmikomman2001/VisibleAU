...
# Version: 1.0 | Built against: LLD v8.65 (REVIEWED) | Sprint: 1 of 9 | Duration: 4 weeks
# Source anchors: LLD §"PHASE 2 SPRINT 1 — PLATFORM FOUNDATION" (lines 4760–5082),
# serve()/registry note (4511), table inventory rows 1–7 (8730–8736), sprint plan (8816).
...
- This repo's Phase 1 code — `lib/audit/runner.ts`, `refresh-audit.ts`, the `audits`
  table, `organizations`, `subscriptions`. Sprint 1 wires INTO these without changing
  their behaviour.
...
```
If either fails, STOP — you have a stale LLD.
...
- 5 platform services (§6): config bundles, budget policy, sampling policy, provider
  capability registry, observability.
...
RLS spec (§8626). Document the disabled-RLS decision in each config schema file's header
comment so a future audit doesn't "fix" it.
...
DO NOTHING`.
`db/seed/provider-market-capabilities.ts` — insert the 4 AU_EN/en-AU providers exactly as
the LLD enumerates (LLD 4908–4916), all `is_enabled=true` at seed time: openai/gpt-4o
(fan-out 12), anthropic/claude-3-5-sonnet (10), google/gemini-1.5-pro (12),
perplexity/pplx-70b-online (8, supports_query_fan_out=false). Use the documented capability
flags per row; `ON CONFLICT (provider_key, model_key, market_code, locale) DO NOTHING`.
...
>    (audit_cost_snapshots: audit_id CASCADE, budget_policy_id SET NULL).
...
grep -cE "CREATE TABLE (config_bundle_cache|market_ai_budget_policies|sampling_policies|metric_quality_gates|prompt_pack_coverage|provider_market_capabilities|audit_cost_snapshots)" db/migrations/*sprint1*.sql   # → 7
...
- v1.0 — Initial Sprint 1 prompt, generated single-pass against LLD v8.65 (REVIEWED).
  ...
  the master plan §7. No reviewer findings applied yet.
