...
# Version: 1.0 | Built against: LLD v8.65 (REVIEWED-r2, verified) | Sprint: 2 of 9 | 4 weeks
...
CORRECTION`). If the version is not 8.65 or the marker count is 0, STOP — stale LLD.
...
- 5 workflow lib modules (§6) + the shared `progress-summary` and wins-feed helpers (§6.6).
- API routes (§9): task queue/create/update/complete, drafts list/generate/approve,
  cross-brand task queue, wins feed, progress summary.
...
└── index.ts
lib/communication/
└── wins-feed.ts                // §6.6 derived wins feed (Phase A: 5 win types)
...
app/api/brands/[id]/wins/route.ts · brands/[id]/progress/route.ts
...
v8.28). `draft_type` TEXT (the full enum at LLD 7890; underscores, e.g.
'wikipedia_article'). `content_format` TEXT NOT NULL — **GAP 8** enum (listicle|how_to_guide
...
- **wins-feed.ts** (LLD 7770, GET /api/brands/[id]/wins, Starter+) — derived, read-only.
  **Phase A (this sprint) implements 5 win types** using Phase 1 + S2/S3 data:
  new_citation, new_engine_coverage, visibility_up, competitor_down, gap_closed (the latter
  from remediation_tasks). visibility_up/competitor_down read S3 tables — guard for their
  absence until S3 lands (return only the available types). `trust_improved` is added in
  S5. ORDER BY detected_at DESC; default LIMIT 20, `?limit=N` max 50 (PA-01).
...
gradient, animations). Then build the shared components at their prototype anchors,
preserving tokens/states exactly: **LayerBadge** (492), **IntelCard** (527; supports the
optional `unit` prop and a neutral `±0` delta pill), **MetricRow** (596), **SectionHeader**
(630), **TierGate** (653; the lock-overlay uses `backdropFilter`, not the corrupted name),
**EmptyState** (691), **StatusBadge** (719; renders the §0.5 statuses), **PriorityBadge**
(754; high/med/low pill is a DERIVED impact band — danger/warning/info — NOT the raw INTEGER
priority column), **ConfidenceBadge** (773). Global ARIA build rules (FIX 13): icon-only
buttons get `aria-label`; tablists get role="tablist"/"tab"/aria-selected; score bars get
role="img" + aria-label; decorative icons `aria-hidden`.

### 6U.2 Workflow hub — WorkflowHub (prototype 2096)
Reproduce the hub layout: LayerBadge "workflow"; the 3 stat cards (Open tasks / In progress
/ Done this month) driven by real counts (a stat card MUST match its task list); the
"Generate draft" and "New task" quick actions (Generate draft → `onNav('content-draft')`
in the prototype = the drafts route here). Both themes; loading/empty/error states.
tabular-nums on the counts.

### 6U.3 Tasks kanban — task-kanban.tsx / task-card.tsx
Columns Open → In Progress → Review → Done (mapping the §0.5 enum: ready_for_review =
"Review", complete = "Done"). task-card shows status, assigned, due, the PriorityBadge
(derived band), and lift-indicator (score before→after) once a re-audit has run. ARIA:
the board is navigable; cards are buttons with accessible names.
...
title"/"Draft body" (body aria-multiline). content-format-badge shows the GAP 8 format +
engine rationale (`format_recommendation_reason`). Approve/Reject controls drive the
content_drafts.status enum. StatusBadge renders draft/approved/published/rejected.

### 6U.5 Dashboard increment — EnhancedDashboard base (prototype 1061)
This sprint builds the dashboard **shell + the "Work Completed / Measured Impact"
two-state card** (prototype ~856 region) driven by progress-summary.ts. **TWO-STATE
DISPLAY (v8.32):** "Work Completed" = COUNT of complete tasks this month (shown
immediately); "Measured Impact" = SUM(lift_achieved) where score_after IS NOT NULL, else
"Validation audit scheduled — measured impact pending" (never a fabricated/zero lift).
The SoV strip (S3) and the Autopilot tracker + Health Check banner (S9) are **out of scope
this sprint** — leave their regions for those sprints. Use the LLD "4 of 11 gaps closed"
framing for the count; do not invent a competing number.
...
>    progress-summary.ts and wins-feed.ts (Phase A: 5 win types, guard S3-sourced types).
...
>    create route emits 'draft/generate'. wins feed paginated (LIMIT 20, ?limit max 50).
>
...
- `wins-feed.test.ts` — Phase A returns the 5 win types; gracefully omits S3-sourced types
  when those tables are empty; LIMIT 20 default, ?limit caps at 50.
...
- **Forgetting RLS on any of the 3 tables** (all carry organization_id) or the MI-01
  idempotency guards.
- **wins-feed crashing when S3 tables are absent** — Phase A must degrade to available types.
...
via ALTER; lights up the SoV dashboard strip; and completes wins-feed visibility_up/
competitor_down. Sprint 3 requires: this sprint's remediation_tasks (gap-spawned tasks land
