...
# Version: 1.0 | Built against: LLD v8.66 (REVIEWED-r2) | Sprint: 6 of 9 | 4 weeks
...
Sprints 1–5 merged. S6 reads S1's services + `selectModel()`, **reuses the Sprint 7 Playwright
crawler** (`lib/crawler/index.ts` — do NOT build a second crawler; if S7 isn't built yet, this
sprint may need the crawler scaffolded — see §13), and reads S3/S5 tables for the
agent-readiness authority + local-trust composites. **Close the last two S4 slots:** wire
...
- 5 Inngest functions (§8): crawler-log-ingest, content-structure-audit (weekly),
  ...
  `serve()`.
- 8 lib modules (§6) under `lib/retrieval/` + the local-ai-trust-scorer (lib/platform).
...
├── score-agent-readiness.ts    // §8.4 event 'technical-audit/complete', emits 'agent/readiness-scored'
...
- **citation-probability-scorer [GAP 8]** (LLD 5260): the headline score — content_format
  (how_to_guide +0.18 … product_page +0.02) + answer_capsule (0–0.25) + freshness (+0.10/+0.05/
  0) + is_entity_home_candidate (+0.08) + optimal_passage_count≥3 (+0.05) + outbound_citation
  (0/+0.03/+0.06/+0.09) + has_author_attribution (+0.04); ~0.85 practical ceiling. This is the
...
- **local_ai_trust_score /100** (lib/platform/local-ai-trust-scorer.ts, §0.5): gmb×0.25 +
  ...
  local_seo_results (Sprint 8 — guard for absence) + brand_entity_scores (S5) +
  citation_source_intelligence (S5). If local_seo_results isn't built yet, compute the
  available components and treat gmb/nap as absent (CPR-01-style), or defer the gmb/nap terms.
...
passive log-upload path per parsed line). Parses → UPSERT crawler_visit_logs; sets
...
- `agent-readiness.test.ts` — each of the 5 dimension formulas at its thresholds; total /100;
  entity_clarity_score is independent of score_of_10.
...
# APPEND-ONLY agent_readiness (no ON CONFLICT)
grep -ic "on conflict\|onConflict" db/schema/agent-readiness-scores.ts                # → 0
...
# entity_clarity_score is NOT score_of_10
grep -Rc "score_of_10" lib/retrieval/agent-readiness.ts                               # → 0
...
- **Building a second crawler.** content-structure-audit REUSES lib/crawler/index.ts (Sprint 7
  Playwright). If S7 isn't built yet, scaffold the shared crawler — do NOT fork a parallel one.
...
comparison_prompt_results) — and crucially **builds the canonical `lib/crawler/index.ts`
Playwright infrastructure + technical-audit-run.ts** that S5/S6 already depend on, so confirm
those emit `technical-audit.complete` (dot) AND `technical-audit/complete` (slash). It also
