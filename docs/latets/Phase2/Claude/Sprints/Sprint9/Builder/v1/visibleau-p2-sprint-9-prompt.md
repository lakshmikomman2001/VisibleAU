...
# Version: 1.0 | Built against: LLD v8.67 (REVIEWED-r2) | Sprint: 9 of 9 | 3 weeks
...
- **BRAND-ACCESS ENFORCEMENT (carried from S8b-01):** any S9 route that is brand-scoped
  (`/api/brands/[id]/…`) MUST call the S8 `assertBrandAccess(user, brandId)` gate, and the
  Autopilot "Execute"/approve action MUST run through it — so the Autopilot cannot act outside a
  member's permitted brands. Do NOT re-introduce an un-enforced brand path.
...
assertBrandAccess); (5) Re-audit + Measure (the score delta, e.g. "citation rate improved 14%"
from visibility_trends / score_after).
...
pending): the current step highlighted, future steps dashed/pending; error: boundary.
...
The post-first-audit experience packaged as a Health Check (NOT a raw results page): a hero banner
(autopilot gradient — reduced-motion-safe) + **traffic-light per dimension** (Frequency/Position/
Sentiment/Context/Accuracy, each great|good|moderate|poor with the statusColor/statusLabel maps:
great→"Great", good→"Good", moderate→"Needs work", poor→"Critical") + **one recommended action**
(the top-priority gap with its explainability rationale). This is the trial→paid conversion moment.
STATES — loading: dimension skeletons; pre-audit (no audit yet): "Run your first audit to see your
Health Check"; partial (insufficient audits): show with the confidence_note caveat; error: boundary.
**RESPONSIVE:** dimension cards `grid-cols-1 sm:grid-cols-2 lg:grid-cols-5`; the hero stacks on `<md`.
...
>    EXISTING approve action through assertBrandAccess — NOT a new write), and visibility_trends/
>    score_after (the delta). CRITICAL: step.status is PRESENTATIONAL ('done'|'current'|'pending')
>    — do NOT unify it with remediation_tasks.status (the DB enum).
> 3. The AI Visibility Health Check (§6U.3): hero banner (reduced-motion-safe) + traffic-light per
>    dimension (Frequency/Position/Sentiment/Context/Accuracy → great|good|moderate|poor with the
>    statusLabel map) + one recommended action with its explainability rationale.
...
- `health-check.test.ts` — the 5 dimensions + the traffic-light status map; one recommended
  action = the top-priority gap; pre-audit + partial (confidence_note) states.
...
role-ceiling + privilege-audit formalisations; and OQ-1 local_seo_results if/when defined), then
