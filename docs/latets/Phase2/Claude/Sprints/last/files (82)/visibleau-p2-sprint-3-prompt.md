...
# Version: 1.4 | Built against: LLD v8.67 (REVIEWED-r2) | Sprint: 3 of 9 | 4 weeks
...
is below 8.65 (not 8.65 or 8.66) or marker count is 0, STOP — stale LLD.
...
- **THE EXPLAINABILITY CONTRACT (platform-wide, GAP 9 / v8.33 — NON-NEGOTIABLE):** every
  ...
  `rationale` (plain-English WHY, written for a non-technical agency owner), `confidence_label`
  ('High'|'Medium'|'Low'|null), `confidence_note` (null for High; a caveat for Medium/Low), and
  `top_action` (single next step or null). Routes populate these by passing their data through
  **`ExplainabilityService.annotate()` (lib/platform/explainability.ts)** before responding — an
  empty `rationale` FAILS CI (`rationale.length > 30`). **S3 is the FIRST scored sprint, so it
  CREATES `lib/platform/explainability.ts`** (§6.9) — later scored sprints (S5/S6) reuse it ("if
  not already present"). This applies to every scored S3 route in §9 (visibility, topical-gaps,
  citation-failure, competitive-benchmark, wins) — the visibility + SoV + archetype scores are
  exactly the numbers customers see first, so they must carry the Where/Why/What-next/Confidence
  annotation, not ship as raw scores.
...
- 7 visibility lib modules (§6.1–§6.7): sov-calculator, mention-source-divide,
  fan-out-simulator, topical-gap-calculator, citation-source-classifier,
  citation-failure-diagnosis, visibility-trend-aggregator — plus **wins-feed.ts (§6.8,
  Phase A — moved from S2)**.
...
lib/platform/
└── explainability.ts                // §6.9 ExplainabilityService.annotate() — CREATED HERE (first scored sprint); S5/S6 reuse
...
### 6.9 explainability.ts — ExplainabilityService (CREATED HERE — LLD 5556, GAP 9)
The platform-wide explainability service, created in S3 (the first scored sprint; later scored
sprints import it). `ExplainabilityService.annotate(data, { scoreType, brandArchetype })` returns
the data augmented with the contract fields (§0.4): `rationale` (plain-English WHY from a template
keyed on scoreType + brand_archetype, Year-10 reading level — never the words "algorithm",
"heuristic", "statistical", "confidence interval"), `confidence_label` (read from the data's
confidence_label/quality_status), `confidence_note` (null for High; the Medium/Low caveat),
`top_action` (the top-priority open remediation_task for the brand, or null). Every scored S3
route (§9) calls it before responding. An empty `rationale` is a build failure (Vitest:
`rationale.length > 30`). Templates live with the service.
...
### 6U.5 Dashboard SoV strip increment — EnhancedDashboard (~1061; reuse the SoV donut/bar visual from VisibilityHub ~1533)
...
Every route: Better Auth session + org scoping; Zod on params/query; correct codes. **Every
score-bearing route (visibility, topical-gaps, citation-failure, competitive-benchmark, wins)
passes its data through `ExplainabilityService.annotate()` (§6.9) before responding** — the
...
>    "likely linked to:" attribution honesty, LIMIT 20 / ?limit max 50). PLUS (G3-01) **create
>    `lib/platform/explainability.ts` (ExplainabilityService.annotate())** — S3 is the first
>    scored sprint; the platform-wide contract requires every score-bearing route to annotate its
>    response with rationale + confidence_label + confidence_note + top_action before returning
>    (empty rationale fails CI). Wire it into /visibility + the other scored §9 routes; S5/S6 reuse it.
...
- `explainability.test.ts` — ExplainabilityService.annotate() populates rationale (non-empty,
  >30 chars per the contract), confidence_label, confidence_note (null for High; set for
  Medium/Low), and top_action; the /visibility (and other scored S3) route responses include the
  contract fields alongside the scores. (Guards G3-01 — the platform contract.)
...
- `citation-source-classifier.test.ts` — source_type classification of cited sources.
...
```bash
# G3-01: the explainability service is created + scored routes annotate before responding
grep -Rc "annotate(" lib/platform/explainability.ts                                  # → ≥1 (created here)
grep -Rc "ExplainabilityService\|annotate(" app/api/brands/\[id\]/visibility/route.ts # → ≥1 (scored route wires it)
grep -REc "rationale|confidence_label|top_action" lib/platform/explainability.ts     # → ≥1
...
## CHANGELOG
- v1.4 — Gate 3 cross-prompt audit fix applied (G3-01), validated against canon first. The
  platform-wide explainability contract (GAP 9 / v8.33 — 'every score-bearing Phase 2 API response
  MUST include { rationale, confidence_label, confidence_note, top_action }', LLD 5537/5567; empty
  rationale fails CI) was referenced 0× in S3 despite S3 shipping score-bearing routes
  (/visibility, topical-gaps, citation-failure, competitive-benchmark, wins). S6/S9 honoured it;
  S3/S5 silently dropped it — a cross-prompt inconsistency Gate 3 caught. Since the contract uses
  the 'create lib/platform/explainability.ts if not already present' pattern and S3 is the FIRST
  scored sprint, S3 now CREATES ExplainabilityService (§6.9) and wires annotate() into its scored
  routes (§0.4 contract + §4 tree + §9 routes + §10 step 3 + a §11 test + a §12 grep). S5 reuses the
  service (its own G3-01 fix). No structural/feature change; render/annotate wiring only — nothing
  existing touched.
...
- v1.1 — Gate 2 findings applied (reviewer chat), validated against r2 first. S3-01 [LOW-MOD]:
  completed §1's lib enumeration — was 5 visibility modules + wins-feed, now all 7 (§6.1–§6.7,
  adding citation-source-classifier and the central visibility-trend-aggregator) + wins-feed
  (§6.8). S3-02 [LOW]: corrected the §6U.5 dashboard anchor (EnhancedDashboard ~1061, reusing
  VisibilityHub ~1533's SoV visual) — proto 1533 is VisibilityHub, not the dashboard. Added
  the optional citation-source-classifier.test.ts to §11. All prompt-internal, no LLD change.
  Reviewer confirmed the four high-risk areas clean: the unit rule (C2), the two FK ALTERs
  (C4), CPR-01 degradation (C6), and wins-feed Phase A built in full here (C7). NOTE for
  S4–S9: treat §1's module list as a complete enumeration of the §4 tree (this was the third
  recurrence of the §1 under-enumeration pattern — S1-02, S2b-01, S3-01).
