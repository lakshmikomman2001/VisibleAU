...
//   FIX 17 (v8.69): TASK-CREATION ENTRY POINT (TC-01, Sprint 2 manual-testing finding). The
//          flagship Workflow loop (Recommendation → Task → Draft → Approve → Reaudit → Lift) was
//          UNREACHABLE — there was NO working UI path to create a remediation_task. ROOT CAUSE:
//          the creation entry point was UNSPECIFIED in both the LLD and this prototype. The
//          WorkflowHub "New task" button (≈L2173) had NO onClick — decorative only. The Action
//          Center recommendation detail had only "Mark as done"/"Dismiss", no "Create task". The
//          empty states said "create one from a recommendation" but no such action existed. The
//          backend create path DID exist (POST /api/brands/[id]/tasks; task-manager populate-on-
//          create; priority-scorer derivation) — only the UI was missing. RESOLVED DESIGN (now
//          canonical, see LLD TC-01 block): PRIMARY — "Create task" on a recommendation detail →
//          POST {recommendationId} → task-manager populates from the recommendation (effort
//          denormalized; confidence_label + priority DERIVED by priority-scorer; status 'open');
//          idempotency guard (no duplicate per open/in_progress recommendation_id). SECONDARY —
//          WorkflowHub "New task" opens a manual create form (title + effort; no recommendation_id;
//          priority still NOT NULL). Annotated inline at the WorkflowHub "New task" button below.
//          NOTE: status is 'complete' (per FIX 1 + LLD) — the Sprint 2 PROMPT §0.5 erroneously had
//          'completed'; LLD wins. ALSO: /workflow surfaces are URL-only (not in brand nav) — the
//          Sprint 9 orphaned-nav pattern; add nav links when wiring the create flow.
// ============================================================
    ...
    <div className="flex-1 overflow-y-auto" style={{ background: 'var(--bg-base)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px' }}>
        ...
        <div className="flex items-start justify-between mb-8">
          ...
          {/* Quick actions */}
          <div className="flex items-center gap-2">
            {/* TC-01 (v8.69): "New task" — SECONDARY create path. Build: onClick opens a manual
                create form/modal (fields: title required, effort low|medium|high required; no
                recommendation_id). Submit → POST /api/brands/[brandId]/tasks → task appears in the
                kanban Open column; the "Open tasks" stat must stay equal to the list. priority is
                NOT NULL — priority-scorer must yield a valid integer in the no-audit case.
                PRIMARY create path is the "Create task" action on a recommendation detail (Action
                Center), NOT here — POST {recommendationId} → task-manager populates from the
                recommendation (effort denormalized; confidence_label + priority DERIVED; status
                'open'); idempotency guard prevents duplicate tasks per recommendation_id. This
                button currently has no onClick — that is the gap TC-01 fixes. */}
            <button
              className="h-8 px-3 text-[12px] font-medium rounded-md flex items-center gap-1.5"
              style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', color: 'var(--text-secondary)' }}
            >
