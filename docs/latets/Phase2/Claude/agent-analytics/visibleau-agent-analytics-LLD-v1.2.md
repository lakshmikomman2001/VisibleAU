# VisibleAU — Agent Analytics — LLD **v1.2**
### (Layer 1 · Retrieval Intelligence — **extension**, not a new layer)

**Status:** DESIGN — not built. Post-Phase-2. 2 sprints.
**Supersedes:** v1.1 (second-pass audit found 5 further conflicts — AA-C10…C14, see §0.2b) and v1.0 (**WITHDRAWN**). It proposed a new Layer 8, four new tables
...
1. **BUILD the crawler card S9 never delivered** (AA-C13) — the canon-spec'd display: `retrieval` 🟢 *"AI
   ...
   breakdown. **Free tier sees the card (canon LLD 3198); verification + ratio are the paid uplift (§8).**
...
## §8. TIERING (AA-18 — **corrected: crawler logs are a DELIBERATE Free-tier hook**)

**AA-C10/C11 correction.** v1.1 claimed a "carried Free-tier leak" and proposed gating crawler logs to Starter+.
**Both were wrong.** Canon is explicit and deliberate:

> **LLD 3198:** *"**Free + Starter**: `crawler_visit_logs` + `llmstxt_versions` (near-zero cost, real value)"*
> **LLD 3199:** *"Starter: `agent_readiness_scores` FULL (differentiates from Free, drives Growth desire)"*
> **v8.56 / TG-02:** the Retrieval tab gate was **already fixed** Growth → Starter. **There is no open bug.**

Crawler visit logs are an **intentional Free-tier acquisition hook** — near-zero cost to serve, high perceived
value, and the cheapest possible *"your GA4 literally cannot see this"* moment. **This is the same strategy v1.1
independently argued for in AA-19. Canon got there first.** Removing it would have been a self-inflicted wound.

| Tier | What they get |
|---|---|
| **Free** | ✅ **`crawler_visit_logs` — UNCHANGED, still free** (canon LLD 3198). Raw AI-bot visits + `visit_purpose`. **The hook.** |
| **Starter** | ✅ + **verification** (verified/unverified/spoofed) + **log-upload ingestion** + `agent_readiness_scores` FULL |
| **Growth** | ✅ + **crawl-to-referral ratio** + `ai_referral_hits` + **CDN-Shield join** + Logpush + coverage gap + alerts |
| **Agency** | ✅ + unlimited domains + white-label PDF section + cross-client rollup |

**AA-19 (restated):** the *paid uplift* is **trust and attribution**, not access. Free sees *that* bots came;
paid learns *whether they were real* (verification) and *whether they sent anyone back* (the ratio). That is a
cleaner value ladder than gating the raw visits — and it preserves the acquisition hook canon designed.
...
Canon asserts in every revision: *"37 tables, 16 GAPs, serve()=25/25, All 7 Layers intact."* This work changes
**three** of those. **Stated, not smuggled:**
...
| **Tables** | 37 | **40** | +3: `ai_bot_registry`, `ai_bot_ip_ranges`, `ai_referral_hits`. **(`crawler_visit_logs` is ALTERed, not added.)** |
| **serve()** | 25/25 | **28/28** | +3 fns (§10). *(The Visit API is a route, not a function — canon precedent, line 947.)* |
...
**Functions (+3 → serve()=28):** `parse-crawler-log` · `verify-crawler-hits` · `refresh-bot-ip-ranges` (daily
...
- **§7.0 first:** verify what S9 built → **extend** it. Retrieval-hub AA section + setup panel. Tier gate (§8) —
...
- **OQ-A1 — RESOLVED by canon (AA-C10).** Crawler logs are **already free** (LLD 3198) — a deliberate hook. The
  question is no longer *"gate it?"* but *"where is the paid line?"* **v1.2 answer: Free keeps the visits;
  verification is Starter; the ratio + CDN-join are Growth.** *Sri to confirm the line.*
