...
test.describe('Brand create form (/brands/new)', () => {
  ...
  test('navigating to /brands/new shows the create form', async ({ page }) => {
    ...
    await expect(page.getByLabel(/brand name/i).or(page.getByPlaceholder(/bondi plumbing/i))).toBeVisible();
    await expect(page.getByLabel(/domain/i).or(page.getByPlaceholder(/bondiplumbing\.com\.au/i))).toBeVisible();
  });
  ...
  test('fills form and submits — brand appears in list (Sprint 1 §11)', async ({ page }) => {
    ...
    const created = (brands as Array<{ name: string; id: string }>)
      .find((b) => b.name === 'E2E Tradies Brand');
  ...
  test('creates Allied Health brand and shows vertical label in list', async ({ page }) => {
    ...
    await page.getByText(/allied health/i).first().click();
    ...
    const brands = await brandRes.json();
    const created = (brands as Array<{ name: string; id: string }>)
      .find((b) => b.name === 'E2E Allied Health Brand');
    if (created) createdBrandIds.push(created.id);
  });
  ...
  test('domain entered with https:// prefix is stripped in DB', async ({ page }) => {
    ...
    const brandRes = await page.request.get('/api/brands');
    const brands = await brandRes.json();
    const created = (brands as Array<{ name: string; id: string; domain: string }>)
      .find((b) => b.name === 'E2E Domain Strip Brand');
  ...
  test('submitting empty name shows a validation error', async ({ page }) => {
    ...
    const errorVisible = await page.getByText(/required/i)
  ...
  test('free-tier org at 1 brand: API returns 403 "Brand limit reached" (Sprint 1 §11)', async ({ page }) => {
    /**
    ...
    await db.update(schema.organizations).set({ tier: 'free' }).where(eq(schema.organizations.id, orgId));
    try {
      ...
      await db.update(schema.organizations).set({ tier: 'agency' }).where(eq(schema.organizations.id, orgId));
    }
  });
