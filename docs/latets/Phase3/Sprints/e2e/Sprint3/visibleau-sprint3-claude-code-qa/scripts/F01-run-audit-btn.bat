@echo off
REM VisibleAU Sprint 3 QA — F01: Run Audit Button Sprint 3
REM MOCK_SCENARIO=happy_path · Inngest required
REM Usage: scripts\F01-F01-run-audit-btn.bat [--headless]
setlocal enabledelayedexpansion
set FEATURE=F01
set LABEL=Run Audit Button Sprint 3
set SPEC=visibleau-sprint3-claude-code-qa/features/F01-run-audit-btn.spec.ts
set PW_ARGS=--project=chromium --headed
if "%1"=="--headless" set PW_ARGS=--project=chromium
echo.
echo ╔══════════════════════════════════════════════════════════╗
echo ║  VisibleAU Sprint 3 QA — F01                              ║
echo ║  Run Audit Button Sprint 3                               ║
...
echo [F01] Starting pnpm dev (MOCK_SCENARIO=happy_path)...
...
goto WAIT_INN
:INN_READY
echo [F01] Inngest ready. Running tests...
:RUN_TEST
echo.
...
echo.
if %EXIT_CODE% equ 0 (echo ✅ F01 PASSED — Run Audit Button Sprint 3) else (echo ❌ F01 FAILED — open sprint3-qa-report\index.html)
echo.
exit /b %EXIT_CODE%