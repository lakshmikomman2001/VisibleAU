@echo off
REM VisibleAU Sprint 3 QA — F02: AuditRunning multi-engine
REM MOCK_SCENARIO=happy_path · Inngest required
REM Usage: scripts\F02-F02-audit-running-sprint3.bat [--headless]
setlocal enabledelayedexpansion
set FEATURE=F02
set LABEL=AuditRunning multi-engine
set SPEC=visibleau-sprint3-claude-code-qa/features/F02-audit-running-sprint3.spec.ts
set PW_ARGS=--project=chromium --headed
if "%1"=="--headless" set PW_ARGS=--project=chromium
echo.
echo ╔══════════════════════════════════════════════════════════╗
echo ║  VisibleAU Sprint 3 QA — F02                              ║
echo ║  AuditRunning multi-engine                               ║
...
echo [F02] Starting pnpm dev (MOCK_SCENARIO=happy_path)...
...
goto WAIT_INN
:INN_READY
echo [F02] Inngest ready. Running tests...
:RUN_TEST
echo.
...
echo.
if %EXIT_CODE% equ 0 (echo ✅ F02 PASSED — AuditRunning multi-engine) else (echo ❌ F02 FAILED — open sprint3-qa-report\index.html)
echo.
exit /b %EXIT_CODE%