@echo off
REM VisibleAU Sprint 3 QA — F06: rate_limited retry
REM MOCK_SCENARIO=rate_limited · Inngest required
REM Usage: scripts\F06-F06-rate-limited.bat [--headless]
setlocal enabledelayedexpansion
set FEATURE=F06
set LABEL=rate_limited retry
set SPEC=visibleau-sprint3-claude-code-qa/features/F06-rate-limited.spec.ts
set PW_ARGS=--project=chromium --headed
if "%1"=="--headless" set PW_ARGS=--project=chromium
echo.
echo ╔══════════════════════════════════════════════════════════╗
echo ║  VisibleAU Sprint 3 QA — F06                              ║
echo ║  rate_limited retry                                      ║
echo ║  MOCK_SCENARIO=rate_limited                                   ║
echo ╚══════════════════════════════════════════════════════════╝
echo.
...
set NODE_ENV=test
set LLM_MODE=mock
set MOCK_SCENARIO=rate_limited
set E2E_USE_REAL_LLM=false
set INNGEST_EVENT_KEY=local
set INNGEST_SIGNING_KEY=local
echo [F06] Starting pnpm dev (MOCK_SCENARIO=rate_limited)...
...
goto WAIT_INN
:INN_READY
echo [F06] Inngest ready. Running tests...
:RUN_TEST
echo.
...
echo.
if %EXIT_CODE% equ 0 (echo ✅ F06 PASSED — rate_limited retry) else (echo ❌ F06 FAILED — open sprint3-qa-report\index.html)
echo.
exit /b %EXIT_CODE%