...
- **serve() stays 25/25 (hard requirement, CC-1).** This feature adds **no new Inngest function** — non-Local lens audits **reuse the existing audit function with a `lens_id` parameter**, so the 25-function `serve()` invariant is untouched. Do **not** add a `run-lens-audit` function.
- **Table manifest: 37 → 39 (intentional, documented, CC-2).** This post-Phase-2 feature adds exactly two tables (`visibility_lenses`, `lens_scores`); **16 GAPs unchanged**. Any gate-check asserting exactly "37 Phase 2 tables" must be updated to **39** for this feature — a deliberate, recorded change, not silent drift. (If minimising the manifest is preferred, `lens_scores` can be a materialized view and `lens_id` a `TEXT` key referencing config, keeping table additions to **zero** — see CC-2.)
