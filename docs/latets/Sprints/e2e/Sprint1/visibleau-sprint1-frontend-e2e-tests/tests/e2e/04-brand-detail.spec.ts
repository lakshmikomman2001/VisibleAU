...
test.describe('Brand detail page (/brands/[brandId])', () => {
  ...
  test('clicking Edit enters edit mode (Sprint 1 §11)', async ({ page }) => {
    ...
    const inEditMode = await saveBtn.isVisible().catch(() => false)
  ...
  test('clicking Delete shows a confirmation dialog (W8 fix — Sprint 1 prototype)', async ({ page }) => {
    await page.goto(`/brands/${currentBrandId}`);
    ...
    const dialogAppeared = await confirmDialog.isVisible().catch(() => false)
  ...
  test('confirming delete soft-deletes the brand and redirects to /brands (Sprint 1 §11)', async ({ page }) => {
    await page.goto(`/brands/${currentBrandId}`);

    await page.getByRole('button', { name: /delete/i }).click();

    // Confirm the delete — look for the destructive action button
    const confirmBtn = page.getByRole('button', { name: /delete brand|confirm|yes/i })
