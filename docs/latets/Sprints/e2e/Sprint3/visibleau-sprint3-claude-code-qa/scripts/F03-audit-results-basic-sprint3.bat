@echo off
REM VisibleAU Sprint 3 QA — F03: AuditResultsBasic Sprint 3
REM MOCK_SCENARIO=happy_path · Inngest required
REM Usage: scripts\F03-F03-audit-results-basic-sprint3.bat [--headless]
setlocal enabledelayedexpansion
set FEATURE=F03
set LABEL=AuditResultsBasic Sprint 3
set SPEC=visibleau-sprint3-claude-code-qa/features/F03-audit-results-basic-sprint3.spec.ts
set PW_ARGS=--project=chromium --headed
if "%1"=="--headless" set PW_ARGS=--project=chromium
echo.
echo ╔══════════════════════════════════════════════════════════╗
echo ║  VisibleAU Sprint 3 QA — F03                              ║
echo ║  AuditResultsBasic Sprint 3                              ║
...
echo [F03] Starting pnpm dev (MOCK_SCENARIO=happy_path)...
...
goto WAIT_INN
:INN_READY
echo [F03] Inngest ready. Running tests...
:RUN_TEST
echo.
...
echo.
if %EXIT_CODE% equ 0 (echo ✅ F03 PASSED — AuditResultsBasic Sprint 3) else (echo ❌ F03 FAILED — open sprint3-qa-report\index.html)
echo.
exit /b %EXIT_CODE%