@echo off
REM VisibleAU Sprint 3 QA — F04: no_mention scenario
REM MOCK_SCENARIO=no_mention · Inngest required
REM Usage: scripts\F04-F04-no-mention.bat [--headless]
setlocal enabledelayedexpansion
set FEATURE=F04
set LABEL=no_mention scenario
set SPEC=visibleau-sprint3-claude-code-qa/features/F04-no-mention.spec.ts
set PW_ARGS=--project=chromium --headed
if "%1"=="--headless" set PW_ARGS=--project=chromium
echo.
echo ╔══════════════════════════════════════════════════════════╗
echo ║  VisibleAU Sprint 3 QA — F04                              ║
echo ║  no_mention scenario                                     ║
echo ║  MOCK_SCENARIO=no_mention                                     ║
echo ╚══════════════════════════════════════════════════════════╝
echo.
...
goto WAIT_INN
:INN_READY
echo [F04] Inngest ready. Running tests...
:RUN_TEST
echo.
...
echo.
if %EXIT_CODE% equ 0 (echo ✅ F04 PASSED — no_mention scenario) else (echo ❌ F04 FAILED — open sprint3-qa-report\index.html)
echo.
exit /b %EXIT_CODE%