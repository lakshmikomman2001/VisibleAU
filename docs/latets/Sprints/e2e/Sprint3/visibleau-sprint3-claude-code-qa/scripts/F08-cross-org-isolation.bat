@echo off
REM VisibleAU Sprint 3 QA — F08: Cross-org isolation
REM MOCK_SCENARIO=happy_path
REM Usage: scripts\F08-F08-cross-org-isolation.bat [--headless]
setlocal enabledelayedexpansion
set FEATURE=F08
set LABEL=Cross-org isolation
set SPEC=visibleau-sprint3-claude-code-qa/features/F08-cross-org-isolation.spec.ts
set PW_ARGS=--project=chromium --headed
if "%1"=="--headless" set PW_ARGS=--project=chromium
echo.
echo ╔══════════════════════════════════════════════════════════╗
echo ║  VisibleAU Sprint 3 QA — F08                              ║
...
echo [F08] Starting pnpm dev (MOCK_SCENARIO=happy_path)...
start /B "S3-Dev" cmd /c "pnpm dev > sprint3-dev.log 2>&1"
REM No Inngest needed
echo [F08] Waiting for app...
set WAITED=0
:WAIT_APP
...
:CLEANUP
echo.
echo [F08] Stopping servers...
taskkill /F /FI "WINDOWTITLE eq S3-Dev" 2>nul
REM No Inngest to stop
taskkill /F /FI "IMAGENAME eq node.exe" 2>nul
echo.
if %EXIT_CODE% equ 0 (echo ✅ F08 PASSED — Cross-org isolation) else (echo ❌ F08 FAILED — open sprint3-qa-report\index.html)
echo.
exit /b %EXIT_CODE%