@echo off
REM VisibleAU Sprint 3 QA — F09: Failed Audit state
REM MOCK_SCENARIO=happy_path
REM Usage: scripts\F09-F09-failed-audit.bat [--headless]
setlocal enabledelayedexpansion
set FEATURE=F09
set LABEL=Failed Audit state
set SPEC=visibleau-sprint3-claude-code-qa/features/F09-failed-audit.spec.ts
set PW_ARGS=--project=chromium --headed
if "%1"=="--headless" set PW_ARGS=--project=chromium
echo.
echo ╔══════════════════════════════════════════════════════════╗
echo ║  VisibleAU Sprint 3 QA — F09                              ║
echo ║  Failed Audit state                                      ║
...
echo [F09] Starting pnpm dev (MOCK_SCENARIO=happy_path)...
start /B "S3-Dev" cmd /c "pnpm dev > sprint3-dev.log 2>&1"
REM No Inngest needed
echo [F09] Waiting for app...
set WAITED=0
:WAIT_APP
...
:CLEANUP
echo.
echo [F09] Stopping servers...
taskkill /F /FI "WINDOWTITLE eq S3-Dev" 2>nul
REM No Inngest to stop
taskkill /F /FI "IMAGENAME eq node.exe" 2>nul
echo.
if %EXIT_CODE% equ 0 (echo ✅ F09 PASSED — Failed Audit state) else (echo ❌ F09 FAILED — open sprint3-qa-report\index.html)
echo.
exit /b %EXIT_CODE%