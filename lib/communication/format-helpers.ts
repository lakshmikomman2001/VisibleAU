...
const QUADRANT: Record<string, { label: string; meaning: string; action: string }> = {
  recognised_authority: {
    ...
    meaning: "AI engines both mention and cite the brand",
    action: "maintain and defend the position",
  },
  known_but_untrusted: {
    label: "known but untrusted",
    meaning: "the brand is mentioned but rarely cited as a source",
    action: "fix content structure so AI engines trust and cite it",
  },
  niche_authority: {
    label: "niche authority",
    meaning: "the brand is cited when it appears, but mention volume is low",
    action: "expand prompt coverage to surface in more queries",
  },
  invisible: {
    label: "invisible",
    meaning: "the brand is neither mentioned nor cited",
    action: "a full GEO strategy to establish presence",
  },
};
