...
curl -s -b "%COOKIE_FILE%" -o "%TEMP%\test-webhooks-api.json" -w "%%{http_code}" "%URL%" > "%TEMP%\test-http-code.txt" 2>&1
...
node -e "const fs=require('fs');const d=JSON.parse(fs.readFileSync(process.env.TEMP+'/test-webhooks-api.json','utf8'));if(!d||typeof d!=='object'){process.exit(1)}" 2>nul
if errorlevel 1 (
    echo FAIL: Response is not valid JSON
    exit /b 1
)

echo PASS: Webhooks Config API returns valid JSON array
exit /b 0
