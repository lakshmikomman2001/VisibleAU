...
import { expect, testAsUser2, test as testUser1, USER_1, USER_2 } from "./helpers/auth";
import {
  db,
...
testAsUser2.describe("User 2 cannot access User 1 brands (CLAUDE.md §7)", () => {
  ...
  testAsUser2.test("User 2 navigating to Org1 brand URL sees 404 (not 401)", async ({ page }) => {
    ...
    expect(is404 || redirectedAway, "Expected 404 page or redirect away from cross-org brand").toBe(
      true,
    );
  });
