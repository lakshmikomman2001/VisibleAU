...
import { get, getClerkToken, request, TEST_USER_1, TEST_USER_2 } from "./helpers/http";
...
describe("GET /api/brands", () => {
  ...
  it("returns only the current org's brands — not other orgs'", async () => {
    ...
    await seedBrand({
      organizationId: org1Id,
      name: "Parramatta Plumbing",
      domain: "paramattaplumbing.com.au",
      vertical: "tradies",
    });
    await seedBrand({
      organizationId: org2Id,
  ...
  it("excludes soft-deleted brands from the list", async () => {
    const active = await seedBrand({
      organizationId: org1Id,
      name: "Active Brand",
      domain: "activebrand.com.au",
    });
    const deleted = await seedBrand({
      organizationId: org1Id,
      name: "Deleted Brand",
      domain: "deletedbrand.com.au",
    });
