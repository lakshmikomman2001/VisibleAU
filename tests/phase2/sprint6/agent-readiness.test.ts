  ...
  it("returns 0 when all false", () => {
    expect(
      computeTechScore({
        ...
        llmstxtValid: false,
        robotsAllowsCrawlers: false,
        ssrPasses: false,
        aiDiscoveryEndpoints: false,
        pageLoadFast: false,
...
describe("§8.4a re-break assertions", () => {
  it("pricingVisible:true adds +5 vs prose (false)", () => {
    const with_ = computeTaskScore({
      bookingAccessible: false,
      pricingVisible: true,
      serviceAreaDefined: false,
      faqDirectAnswers: 0,
    });
    const without = computeTaskScore({
      bookingAccessible: false,
      pricingVisible: false,
      serviceAreaDefined: false,
      faqDirectAnswers: 0,
