...
echo [F03] Stopping any existing server on port 3000...
...
if !READY!==0 (
  echo [F03] ERROR: Server did not start!
  pause
  exit /b 1
)
...
call pnpm exec playwright test tests/qa/sprint1/features/f03-auth-signup/f03-auth-signup.spec.ts --config tests/qa/sprint1/playwright.config.ts --reporter=list --headed
