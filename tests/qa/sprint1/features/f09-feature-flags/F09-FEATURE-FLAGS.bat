...
cd /d "%~dp0..\..\..\..\..\"
echo [F09] Working directory: %CD%

echo [F09] Loading environment...
...
echo [F09] Stopping any existing server on port 3000...
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (
  taskkill /PID %%P /F >nul 2>&1
)
timeout /t 2 /nobreak >nul

echo [F09] Starting Next.js dev server...
...
if !READY!==0 (
  echo [F09] ERROR: Server did not start!
  pause
  exit /b 1
)
echo [F09] Server ready.

echo [F09] Running feature flags tests...
call pnpm exec playwright test tests/qa/sprint1/features/f09-feature-flags/f09-feature-flags.spec.ts --config tests/qa/sprint1/playwright.config.ts --reporter=list --headed
set TEST_EXIT=%ERRORLEVEL%

echo [F09] Stopping dev server...
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (
  taskkill /PID %%P /F >nul 2>&1
)
