...
cd /d "%~dp0..\..\..\..\..\"
echo [F11] Working directory: %CD%

echo [F11] Loading environment...
...
echo [F11] Stopping any existing server on port 3000...
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (
  taskkill /PID %%P /F >nul 2>&1
)
timeout /t 2 /nobreak >nul

echo [F11] Starting Next.js dev server...
...
if !READY!==0 (
  echo [F11] ERROR: Server did not start!
  pause
  exit /b 1
)
echo [F11] Server ready.

echo [F11] Running Clerk webhook tests...
call pnpm exec playwright test tests/qa/sprint1/features/f11-clerk-webhook/f11-clerk-webhook.spec.ts --config tests/qa/sprint1/playwright.config.ts --reporter=list --headed
set TEST_EXIT=%ERRORLEVEL%

echo [F11] Stopping dev server...
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (
  taskkill /PID %%P /F >nul 2>&1
)
