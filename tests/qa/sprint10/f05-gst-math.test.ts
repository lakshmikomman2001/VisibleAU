...
function displayPrice(
  exGstAud: number,
  opts: { incGst: boolean; interval?: "monthly" | "annual" },
): string {
...
describe("F05: GST math and display (HC2, HG1 — no double-charge)", () => {
  it("F05-01: addGst(90) → 99", () => {
    expect(addGst(90)).toBe(99);
  });
  it("F05-02: addGst(270) → 297", () => {
    expect(addGst(270)).toBe(297);
  });
  it("F05-03: addGst(450) → 495", () => {
    expect(addGst(450)).toBe(495);
  });
  it("F05-04: addGst(1350) → 1485", () => {
    expect(addGst(1350)).toBe(1485);
  });
  it("F05-05: removeGst(99) → 90", () => {
