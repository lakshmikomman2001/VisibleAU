...
cd /d "%~dp0..\..\..\"
echo [F10-S10] Working directory: %CD%
...
echo [F10-S10] POST /api/sample-audit — Full E2E
...
start "" /B cmd /c "pnpm dev > tests\qa\sprint10\logs\f10_sample_audit_api-server.log 2>&1"
set READY=0
...
if !READY!==0 (echo [F10-S10] Server failed! & pause & exit /b 1)
echo [F10-S10] Server ready. Running tests...
call pnpm exec playwright test tests/qa/sprint10/f10-sample-audit-api.spec.ts --config tests/qa/sprint10/playwright.config.ts --reporter=list --headed
set TEST_EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (taskkill /PID %%P /F >nul 2>&1)
if %TEST_EXIT% EQU 0 (echo [F10-S10] PASSED) else (echo [F10-S10] FAILED)
pause
exit /b %TEST_EXIT%