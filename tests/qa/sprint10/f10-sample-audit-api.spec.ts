...
test.afterAll(async () => {
  const [sampleOrg] = await db
    ...
    .where(eq(schema.organizations.slug, "__sample__"))
    .limit(1);
  if (sampleOrg) {
    const auditIds = (
      await db
        ...
        .from(schema.audits)
        .where(eq(schema.audits.organizationId, sampleOrg.id))
    ).map((a) => a.id);
    if (auditIds.length) {
      await db
      ...
      await db
        ...
        .where(inArray(schema.audits.id, auditIds))
        .catch(() => {});
    }
    await db
      .delete(schema.brands)
      .where(eq(schema.brands.organizationId, sampleOrg.id))
      .catch(() => {});
...
test.describe("F10: POST /api/sample-audit — full E2E flow (HC1, HB2)", () => {
  ...
  test('F10-02: audit row created in DB under slug="sample" org (HC1)', async ({ request }) => {
    ...
    const [audit] = await db
      .select({ orgId: schema.audits.organizationId })
      .from(schema.audits)
    ...
    const [org] = await db
      ...
      .from(schema.organizations)
      .where(eq(schema.organizations.id, audit.orgId));
    expect(org?.slug).toBe("__sample__");
  });

  test('F10-03: audit has engines containing "chatgpt" and promptsCount set (HB2)', async ({
    request,
  }) => {
    const res = await request.post("/api/sample-audit", {
    ...
    for (let i = 0; i < 30; i++) {
      const [row] = await db
        ...
        .from(schema.audits)
        .where(eq(schema.audits.id, auditId));
      if (row && row.engines && row.engines.length > 0 && row.promptsCount !== null) {
        audit = row;
        break;
      }
  ...
  test("F10-06: /sample-audit page renders domain input form", async ({ page }) => {
    ...
    const input = page.getByPlaceholder(/domain|yourdomain/i).or(page.getByRole("textbox").first());
  ...
  test("F10-08: 4th request from same IP returns 429 (rate limit)", async ({ request }) => {
