...
import { cleanupOrg, seedOrg, seedUser } from "./shared/seed";
...
test.describe("F11: /pricing page + billing checkout (HA3, HB1, HG1, HM4)", () => {
  ...
  test("F11-05: monthly/annual toggle present", async ({ page }) => {
    ...
    const annualBtn = page
      ...
      .or(page.getByText(/annual|yearly/i))
      .first();
    await expect(annualBtn).toBeVisible({ timeout: 10_000 });
  });

  test("F11-06: POST /api/billing/checkout with valid tier returns {url} or auth redirect", async ({
    request,
  }) => {
  ...
  test("F11-07: POST /api/billing/checkout with unknown tier returns error or auth redirect", async ({
    request,
  }) => {
    const res = await request.post("/api/billing/checkout", {
  ...
  test("F11-08: POST /api/billing/checkout with missing body returns error or auth redirect", async ({
    request,
  }) => {
    const res = await request.post("/api/billing/checkout", {
      data: {},
  ...
  test("F11-10: upgrade CTA button navigates to /pricing or triggers checkout", async ({
    page,
  }) => {
