...
import { cleanupOrg, cleanupWebhookEvent, seedOrg, seedUser } from "./shared/seed";
...
test.beforeAll(async () => {
  const org = await seedOrg({
    ...
    name: "[S10QA F12] Webhook Org",
    tier: "free",
  });
  orgId = org.id;
  await seedUser({ clerkUserId: CLERK_USER, organizationId: orgId, email: `f12_${TS}@test.com` });
});
...
test.describe("F12: Stripe webhook → tier sync E2E (HJ3, HA3, HD2)", () => {
  ...
  test("F12-02: checkout.session.completed with valid signature returns 200", async ({
    request,
  }) => {
  ...
  test("F12-04: organizations.tier updated to starter after checkout (HA3)", async () => {
    ...
    const [org] = await db
      .select({ tier: schema.organizations.tier })
  ...
  test("F12-06: invoice.payment_failed does NOT immediately downgrade org.tier", async ({
    request,
  }) => {
