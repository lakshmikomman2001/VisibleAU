...
cd /d "%~dp0..\..\..\"
echo [F06-S11] Working directory: %CD%
...
echo [F06-S11] Public Pages (About/Privacy/Terms/Pricing) E2E
...
start "" /B cmd /c "pnpm dev > tests\qa\sprint11\logs\f06-server.log 2>&1"
set READY=0
...
if !READY!==0 (echo [F06-S11] Server failed! & pause & exit /b 1)
echo [F06-S11] Server ready. Running tests...
call pnpm exec playwright test tests/qa/sprint11/f06-public-pages.spec.ts --config tests/qa/sprint11/playwright.config.ts --reporter=list
set TEST_EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (taskkill /PID %%P /F >nul 2>&1)
if %TEST_EXIT% EQU 0 (echo [F06-S11] PASSED) else (echo [F06-S11] FAILED)
pause
exit /b %TEST_EXIT%
