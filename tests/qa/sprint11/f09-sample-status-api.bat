...
cd /d "%~dp0..\..\..\"
echo [F09-S11] Working directory: %CD%
...
echo [F09-S11] Sample Audit Status API E2E
...
start "" /B cmd /c "pnpm dev > tests\qa\sprint11\logs\f09-server.log 2>&1"
set READY=0
...
if !READY!==0 (echo [F09-S11] Server failed! & pause & exit /b 1)
echo [F09-S11] Server ready. Running tests...
call pnpm exec playwright test tests/qa/sprint11/f09-sample-status-api.spec.ts --config tests/qa/sprint11/playwright.config.ts --reporter=list
set TEST_EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (taskkill /PID %%P /F >nul 2>&1)
if %TEST_EXIT% EQU 0 (echo [F09-S11] PASSED) else (echo [F09-S11] FAILED)
pause
exit /b %TEST_EXIT%
