...
cd /d "%~dp0..\..\..\..\..\"
echo [F04-S2] Working directory: %CD%
...
start "" /B cmd /c "pnpm dev > tests\qa\sprint2\logs\f04-server.log 2>&1"
set READY=0
...
if !READY!==0 (echo [F04-S2] Server failed! & pause & exit /b 1)
echo [F04-S2] Server ready. Running tests...
call pnpm exec playwright test tests/qa/sprint2/features/f04-brand-detection/f04-brand-detection.spec.ts --config tests/qa/sprint2/playwright.config.ts --reporter=list --headed
set TEST_EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (taskkill /PID %%P /F >nul 2>&1)
if %TEST_EXIT% EQU 0 (echo [F04-S2] PASSED) else (echo [F04-S2] FAILED)
pause
exit /b %TEST_EXIT%
