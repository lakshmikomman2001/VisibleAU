...
start /B cmd /c "set LLM_MODE=mock&& call pnpm dev > tests\qa\sprint5\logs\f01-server.log 2>&1"
:WAIT_F01
...
call pnpm exec playwright test tests/qa/sprint5/features/f01-schema/f01-schema.spec.ts --config tests/qa/sprint5/playwright.config.ts --reporter=list --headed
set EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano ^| findstr ":3000"') do taskkill /PID %%P /F >nul 2>&1
