...
start /B cmd /c "set LLM_MODE=mock&& call pnpm dev > tests\qa\sprint5\logs\f02-server.log 2>&1"
:WAIT_F02
...
call pnpm exec playwright test tests/qa/sprint5/features/f02-seed-data/f02-seed-data.spec.ts --config tests/qa/sprint5/playwright.config.ts --reporter=list --headed
set EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano ^| findstr ":3000"') do taskkill /PID %%P /F >nul 2>&1
