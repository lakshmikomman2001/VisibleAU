...
start /B cmd /c "set LLM_MODE=mock&& call pnpm dev > tests\qa\sprint6\logs\f03-server.log 2>&1"
:WAIT_F03
...
call pnpm exec playwright test tests/qa/sprint6/features/f03-anti-patterns/f03-anti-patterns.spec.ts --config tests/qa/sprint6/playwright.config.ts --reporter=list --headed
set EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano ^| findstr ":3000"') do taskkill /PID %%P /F >nul 2>&1
