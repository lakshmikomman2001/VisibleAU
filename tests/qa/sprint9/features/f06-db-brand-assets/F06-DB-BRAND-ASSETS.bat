...
cd /d "%~dp0..\..\..\..\..\"
echo [F06-S9] Working directory: %CD%
...
start "" /B cmd /c "pnpm dev > tests\qa\sprint9\logs\f06_db_brand_assets-server.log 2>&1"
set READY=0
...
if !READY!==0 (echo [F06-S9] Server failed! & pause & exit /b 1)
echo [F06-S9] Server ready. Running tests...
call pnpm exec playwright test tests/qa/sprint9/features/f06-db-brand-assets/f06-agency-branding-e2e.spec.ts --config tests/qa/sprint9/playwright.config.ts --reporter=list --headed
set TEST_EXIT=%ERRORLEVEL%
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr "LISTENING" ^| findstr ":3000"') do (taskkill /PID %%P /F >nul 2>&1)
if %TEST_EXIT% EQU 0 (echo [F06-S9] PASSED) else (echo [F06-S9] FAILED)
pause
exit /b %TEST_EXIT%