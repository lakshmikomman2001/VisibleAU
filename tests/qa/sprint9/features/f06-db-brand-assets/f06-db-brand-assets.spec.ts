...
import { cleanupOrg, seedAgencyBrandAsset, seedBrand, seedOrg, seedUser } from "../../shared/seed";
...
beforeAll(async () => {
  ...
  org1Id = o1.id;
  await seedUser({
    clerkUserId: `user_s9qa_f06_1_${Date.now()}`,
    organizationId: org1Id,
    email: `f06_1_${Date.now()}@test.local`,
  });
  ...
  org2Id = o2.id;
  await seedUser({
    clerkUserId: `user_s9qa_f06_2_${Date.now()}`,
    organizationId: org2Id,
    email: `f06_2_${Date.now()}@test.local`,
  });
});
afterAll(async () => {
...
describe("[S9QA] F06 — DB CRUD: agencyBrandAssets", () => {
  it("F06-01: insert org-default asset (brandId=null) -> id returned", async () => {
    const r = await seedAgencyBrandAsset({
      organizationId: org1Id,
      brandId: null,
      primaryColor: "#112233",
    });
  ...
  it("F06-03: SELECT org-default via isNull filter returns row", async () => {
    const rows = await db
      ...
      .where(
        and(
          eq(schema.agencyBrandAssets.organizationId, org1Id),
  ...
  it("F06-04: upsert org-default — second call updates colour, no duplicate row (GA3)", async () => {
    ...
    const rows = await db
      .select({ pc: schema.agencyBrandAssets.primaryColor })
      .from(schema.agencyBrandAssets)
      .where(
        and(
          eq(schema.agencyBrandAssets.organizationId, org1Id),
  ...
  it("F06-05: agencyName stored correctly", async () => {
    const rows = await db
      .select({ name: schema.agencyBrandAssets.agencyName })
      .from(schema.agencyBrandAssets)
  ...
  it("F06-06: updatedAt is a Date (never null — GA2 pattern)", async () => {
    const rows = await db
      .select({ u: schema.agencyBrandAssets.updatedAt })
      .from(schema.agencyBrandAssets)
      .where(eq(schema.agencyBrandAssets.organizationId, org1Id));
  ...
  it("F06-08: DELETE all org1 brand assets leaves zero rows", async () => {
    await db
      .delete(schema.agencyBrandAssets)
      .where(eq(schema.agencyBrandAssets.organizationId, org1Id));
    const rows = await db
      .select()
      .from(schema.agencyBrandAssets)
