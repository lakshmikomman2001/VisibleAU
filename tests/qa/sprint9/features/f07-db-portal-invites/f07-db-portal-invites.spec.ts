...
beforeAll(async () => {
  ...
  org1Id = o1.id;
  await seedUser({
    clerkUserId: `user_s9qa_f07_1_${Date.now()}`,
    organizationId: org1Id,
    email: `f07_1_${Date.now()}@test.local`,
  });
  ...
  org2Id = o2.id;
  await seedUser({
    clerkUserId: `user_s9qa_f07_2_${Date.now()}`,
    organizationId: org2Id,
    email: `f07_2_${Date.now()}@test.local`,
  });
});
afterAll(async () => {
...
describe("[S9QA] F07 — DB CRUD: clientPortalInvites + views", () => {
  ...
  it("F07-02: inviteeEmail=null creates invite without email (GH3)", async () => {
    const i = await seedClientPortalInvite({
      organizationId: org1Id,
      brandId: brand1Id,
      inviteeEmail: null,
    });
  ...
  it("F07-04: revoke — isRevoked becomes true (GF1 boolean)", async () => {
    await db
      .update(schema.clientPortalInvites)
      .set({ isRevoked: true, status: "revoked" })
      .where(eq(schema.clientPortalInvites.id, activeInviteId));
    const [row] = await db
      .select({ isRevoked: schema.clientPortalInvites.isRevoked })
  ...
  it("F07-05: expired invite — expiresAt in past (token expired)", async () => {
    ...
    const [row] = await db
      .select({ expiresAt: schema.clientPortalInvites.expiresAt })
      .from(schema.clientPortalInvites)
  ...
  it("F07-06: isRevoked=true blocks access even with future expiresAt (T4)", async () => {
    ...
    const [row] = await db
      .select({
        isRevoked: schema.clientPortalInvites.isRevoked,
  ...
  it("F07-09: cleanupOrg deletes views before invites — no FK violation", async () => {
    const tmpO = await seedOrg({
      clerkOrgId: `org_s9qa_f07_tmp_${Date.now()}`,
      name: "[S9QA] F07 Tmp",
    });
    try {
      ...
      await cleanupOrg(tmpO.id).catch(() => {});
      throw e;
    }
  });
});
