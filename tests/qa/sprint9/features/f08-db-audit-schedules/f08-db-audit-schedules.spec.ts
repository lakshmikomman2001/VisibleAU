...
beforeAll(async () => {
  ...
  orgId = o.id;
  await seedUser({
    clerkUserId: `user_s9qa_f08_${Date.now()}`,
    organizationId: orgId,
    email: `f08_${Date.now()}@test.local`,
  });
  ...
  const due = await seedAuditSchedule({
    organizationId: orgId,
    brandId: brand1Id,
    frequency: "daily",
  ...
  const future = await seedAuditSchedule({
    organizationId: orgId,
    brandId: brand2Id,
    frequency: "weekly",
...
describe("[S9QA] F08 — DB CRUD: auditSchedules", () => {
  ...
  it("F08-06: quota_exceeded status stored correctly", async () => {
    ...
    const [s] = await db
      .select({ status: schema.auditSchedules.status })
      .from(schema.auditSchedules)
      .where(eq(schema.auditSchedules.id, futureId));
  ...
  it("F08-07: all schedule rows have updatedAt set (GA2 NOT NULL)", async () => {
    const rows = await db
      .select({ u: schema.auditSchedules.updatedAt })
      .from(schema.auditSchedules)
