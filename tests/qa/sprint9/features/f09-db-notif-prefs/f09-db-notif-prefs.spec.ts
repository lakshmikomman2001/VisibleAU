...
import { cleanupOrg, seedNotificationPrefs, seedOrg, seedUser } from "../../shared/seed";
...
beforeAll(async () => {
  ...
  orgId = o.id;
  await seedUser({
    clerkUserId: `user_s9qa_f09_${Date.now()}`,
...
describe("[S9QA] F09 — DB CRUD: notificationPreferences (T3)", () => {
  it("F09-01: create prefs row — weeklyDigest=true, digestEmail set", async () => {
    const n = await seedNotificationPrefs({
      organizationId: orgId,
      email: testEmail,
      weeklyDigest: true,
    });
  ...
  it("F09-02: upsert — calling seedNotificationPrefs twice does NOT create two rows", async () => {
    await seedNotificationPrefs({ organizationId: orgId, email: testEmail, weeklyDigest: false });
    const rows = await db
      .select()
  ...
  it("F09-06: updatedAt is a Date on all rows", async () => {
    const [row] = await db
      .select({ u: schema.notificationPreferences.updatedAt })
      .from(schema.notificationPreferences)
      .where(eq(schema.notificationPreferences.organizationId, orgId));
