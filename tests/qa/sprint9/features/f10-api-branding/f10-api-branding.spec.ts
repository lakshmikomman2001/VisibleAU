...
beforeAll(async () => {
  ...
  orgId = o.id;
  await seedUser({
    clerkUserId: `user_s9qa_f10_${Date.now()}`,
    organizationId: orgId,
    email: `f10_${Date.now()}@test.local`,
  });
...
describe("[S9QA] F10 — agency branding GET/PATCH (GG3) — DB-level", () => {
  it("F10-01: org-default branding asset exists after seed", async () => {
    const rows = await db
      ...
      .where(
        and(
          eq(schema.agencyBrandAssets.organizationId, orgId),
  ...
  it("F10-02: fresh org has zero branding rows", async () => {
    const tmpO = await seedOrg({
      clerkOrgId: `org_s9qa_f10_tmp_${Date.now()}`,
      name: "[S9QA] F10 Tmp",
      tier: "free",
    });
    try {
  ...
  it("F10-04: PATCH persists to DB — direct DB read confirms new values", async () => {
    await db
      .update(schema.agencyBrandAssets)
