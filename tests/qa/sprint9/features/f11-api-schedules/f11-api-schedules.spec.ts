...
import { cleanupOrg, seedAuditSchedule, seedBrand, seedOrg, seedUser } from "../../shared/seed";
...
beforeAll(async () => {
  ...
  org1Id = o1.id;
  await seedUser({
    clerkUserId: `user_s9qa_f11_1_${Date.now()}`,
    organizationId: org1Id,
    email: `f11_1_${Date.now()}@test.local`,
  });
  const b = await seedBrand({ organizationId: org1Id, name: "[S9QA] F11 Brand" });
  brandId = b.id;
  const s = await seedAuditSchedule({
    organizationId: org1Id,
    brandId,
    frequency: "weekly",
    status: "active",
  ...
  const o2 = await seedOrg({
    clerkOrgId: `org_s9qa_f11_2_${Date.now()}`,
    name: "[S9QA] F11 Org2",
    tier: "free",
  });
  org2Id = o2.id;
  await seedUser({
    clerkUserId: `user_s9qa_f11_2_${Date.now()}`,
    organizationId: org2Id,
    email: `f11_2_${Date.now()}@test.local`,
  });
});
afterAll(async () => {
...
describe("[S9QA] F11 — audit-schedules GET/PATCH (GG2) — DB-level", () => {
  ...
  it("F11-05: PATCH pause persists — updatedAt refreshed (GA2)", async () => {
    const [row] = await db
      .select({ updatedAt: schema.auditSchedules.updatedAt })
      .from(schema.auditSchedules)
      .where(eq(schema.auditSchedules.id, scheduleId));
  ...
  it("F11-07: cron-due query returns only active schedules with past nextRunAt", async () => {
    ...
    await db
      ...
      .where(eq(schema.auditSchedules.id, scheduleId));
    const due = await db
      .select({ id: schema.auditSchedules.id })
      .from(schema.auditSchedules)
      .where(
        and(
