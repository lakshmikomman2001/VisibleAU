...
beforeAll(async () => {
  ...
  orgId = o.id;
  await seedUser({
    clerkUserId: `user_s9qa_f12_${Date.now()}`,
  ...
  const active = await seedClientPortalInvite({
    organizationId: orgId,
    brandId,
    isRevoked: false,
  ...
  const expired = await seedClientPortalInvite({
    organizationId: orgId,
    brandId,
    isRevoked: false,
  ...
  const revoked = await seedClientPortalInvite({
    organizationId: orgId,
    brandId,
    isRevoked: true,
    expiresAt: new Date(Date.now() + 30 * 86_400_000),
