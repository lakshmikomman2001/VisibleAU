...
beforeAll(async () => {
  ...
  orgId = o.id;
  await seedUser({
    clerkUserId: `user_s9qa_f13_${Date.now()}`,
    organizationId: orgId,
    email: testEmail,
  });
  const b1 = await seedBrand({
    organizationId: orgId,
    name: "[S9QA] F13 Brand1",
    clientTag: "AcmeCorp",
  });
  brand1Id = b1.id;
  const b2 = await seedBrand({
    organizationId: orgId,
    name: "[S9QA] F13 Brand2",
    clientTag: "AcmeCorp",
  });
  brand2Id = b2.id;
  await seedAudit({
    ...
    brandId: brand1Id,
    auditNumber: 1,
    scoreComposite: "71.50",
  });
  await seedAudit({
    organizationId: orgId,
    brandId: brand2Id,
    auditNumber: 2,
    scoreComposite: "64.20",
  });
  await seedNotificationPrefs({ organizationId: orgId, email: testEmail, weeklyDigest: true });
});
afterAll(async () => {
...
describe("[S9QA] F13 — bulk-export + notification-prefs (GH4, GH1) — DB-level", () => {
  ...
  it("F13-08: org with no prefs has zero rows (GH1 defaults)", async () => {
    const tmpO = await seedOrg({
      clerkOrgId: `org_s9qa_f13_noprefs_${Date.now()}`,
      name: "[S9QA] F13 NoPref",
      tier: "free",
    });
    try {
      const rows = await db
  ...
  it("F13-09: PATCH weeklyDigest updates in DB", async () => {
    await db
      ...
      .where(eq(schema.notificationPreferences.organizationId, orgId));
    const [row] = await db
      .select({ weeklyDigest: schema.notificationPreferences.weeklyDigest })
      .from(schema.notificationPreferences)
      .where(eq(schema.notificationPreferences.organizationId, orgId));
