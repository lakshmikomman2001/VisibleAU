...
export async function seedAgencyBrandAsset(p: {
  ...
  if (p.brandId === null) {
    const existing = await db
      .select({ id: schema.agencyBrandAssets.id })
  ...
  const [r] = await db
    .insert(schema.agencyBrandAssets)
    .values({
      organizationId: p.organizationId,
      brandId: p.brandId,
      primaryColor: p.primaryColor ?? "#003366",
...
export async function seedClientPortalInvite(p: {
  ...
  const [i] = await db
    .insert(schema.clientPortalInvites)
    .values({
      organizationId: p.organizationId,
      brandId: p.brandId,
      inviteToken: token,
...
export async function cleanupOrg(orgId: string) {
  await db
    .delete(schema.clientPortalViews)
    .where(eq(schema.clientPortalViews.organizationId, orgId))
    .catch(() => {});
  await db
    .delete(schema.clientPortalInvites)
  ...
  if (brands.length) {
    ...
    await db
      ...
      .where(inArray(schema.audits.brandId, bids))
      .catch(() => {});
  }
  await db
